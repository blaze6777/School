const GAME_VERSION="9.7.0";
const GAME_BUILD="2026-09-23 08:34 ET";
...
/* ================= V9.1.1 INTERACTION + EVENT QUEUE ================= */
function v911TalkEmployee(e){
 if(!e)return;
 ensureV91State();
 state.principalLocation=e.room||currentEmployeeLocation(e)||'OFFICE';
 if(state.play)state.play.principalLoc=state.principalLocation;
 state.simMinutes=Math.min(1080,state.simMinutes+3);
 let category=e.category||'', topic='',details='',choices=[];
 if(category==='Cafeteria'){
   topic=pick(['Lunch service check-in','Kitchen operations','Student lunch flow','Supplies and equipment']);
   details=pick([
    `${e.name} says lunch service is on schedule but the team is watching the serving-line pace as the larger grades arrive.`,
    `${e.name} mentions one piece of kitchen equipment has been acting up intermittently, though service has continued.`,
    `${e.name} says inventory is generally good but a few high-use items will need to be reordered soon.`,
    `${e.name} says student behavior in the cafeteria has been manageable, with a couple of tables needing extra reminders.`
   ]);
   choices=['Thank them and ask what they need','Walk lunch service with them','Create a follow-up task','Just check in and move on'];
 }else if(category==='Specials'){
   topic=pick(['Specials schedule check-in','Class transition concern','Equipment / supply need','Student engagement']);
   details=pick([
    `${e.name} says most classes are going well but transitions between homerooms and specials have been a little rushed.`,
    `${e.name} has a small equipment or supply need that would make instruction smoother.`,
    `${e.name} mentions one grade level has needed more redirection than usual during specials.`,
    `${e.name} says students are especially engaged in the current unit and shares something they are proud of.`
   ]);
   choices=['Ask what support would help','Visit during the next class','Create a follow-up task','Thank them for the update'];
 }else if(category==='Teacher'){
   topic=pick(['Classroom check-in','Student concern','Schedule question','Instructional support']);
   details=pick([
    `${e.name} says the class is generally going well, but one student may need a little more support.`,
    `${e.name} asks whether you can revisit a schedule issue when you have time.`,
    `${e.name} shares that the class had a strong lesson today and students responded well.`,
    `${e.name} says transitions have been harder this week and would appreciate another set of eyes.`
   ]);
   choices=['Tell me more','I’ll follow up today','I’ll stop back during instruction','Thanks for the check-in'];
 }else if(category==='Operations'){
   topic='Building operations check-in';
   details=pick([`${e.name} says the building is in good shape overall but points out a small area they are watching.`,`${e.name} gives you a quick update on cleaning, room setups, and open work orders.`]);
   choices=['Ask what needs attention','Walk the area together','Create follow-up task','Thank them'];
 }else{
   topic='Staff check-in';
   details=pick([`${e.name} says the day is going smoothly and appreciates you stopping by.`,`${e.name} mentions a routine issue they wanted you to be aware of.`]);
   choices=['Tell me more','Anything you need from me?','Create follow-up task','Thanks for the update'];
 }
 openModal(`💬 Check In — ${e.name}`,`<div class="app-sheet">
   <h3>${e.name}</h3><p>${e.position} • ${locationLabel(e.room||currentEmployeeLocation(e))}</p>
   <div class="app-section"><strong>${topic}</strong><p>${details}</p></div>
   <div class="actions">${choices.map((c,i)=>`<button class="${i===0?'primary':'secondary'}" data-v911-talk="${i}">${c}</button>`).join('')}</div>
 </div>`);
 document.querySelectorAll('[data-v911-talk]').forEach(b=>b.onclick=()=>{
   let i=Number(b.dataset.v911Talk),choice=choices[i],record=`Principal checked in with ${e.name}: ${topic}. Response: ${choice}.`;
   state.simMinutes=Math.min(1080,state.simMinutes+(i===0?7:i===1?5:i===2?4:2));
   e.memory=e.memory||[];e.memory.unshift(`${state.date}: ${record}`);
   state.schoolHistory.unshift({date:state.date,text:record});
   if(/follow up|follow-up|need|support/i.test(choice))addTask(`Follow up with ${e.name}: ${topic}`,'Today','Normal');
   if(/stop back|visit during/i.test(choice))v91Promise(e.name,`Return for ${topic.toLowerCase()}`,'Today');
   closeModal();toast(`Check-in with ${e.name} recorded.`);render();
 });
}

const v83RoomInspectorV911Old=v83RoomInspector;
v83RoomInspector=function(){
 let box=$('roomInspector'),id=state.selectedRoom;
 if(!box)return;
 if(!id){box.innerHTML='<span class="muted">Select a room to see the live school.</span>';return;}
 let r=roomById(id);if(!r)return;
 let people=peopleAt(id),teacher=teacherForRoom(id);
 box.innerHTML=`<h3>${id} — ${r.name}${r.grade?` / Grade ${r.grade}`:''}</h3>
 <div class="live-room-banner"><strong>${liveActivityForRoom(r)}</strong><span>${mins12(state.simMinutes)} • ${liveRoomSummary(id)}</span></div>
 <div class="inspector-grid"><div class="inspector-stat"><span>Students Here</span><strong>${people.students.length}</strong></div><div class="inspector-stat"><span>Staff Here</span><strong>${people.staff.length}</strong></div><div class="inspector-stat"><span>Clean</span><strong>${r.cleanliness}%</strong></div><div class="inspector-stat"><span>Temp</span><strong>${r.temp}°F</strong></div></div>
 ${r.grade?`<div class="compact-item"><strong>${teacher?teacher.name:'Vacant classroom'}</strong><br>${teacher?`${teacher.position} • ${teacher.experience} yrs experience`:'No teacher assigned'}</div>`:''}
 <div class="live-people-list"><strong>Currently here</strong>${people.staff.slice(0,8).map(x=>`<div class="room-person-row"><span>👩‍🏫 ${x.name} — ${x.position}</span><button class="secondary v911-room-talk" data-emp="${x.id}">💬 Check In</button></div>`).join('')}${people.students.slice(0,10).map(x=>`<div>🎒 ${x.first} ${x.last} — Grade ${x.grade}</div>`).join('')}${people.students.length>10?`<div class="muted">+ ${people.students.length-10} more students</div>`:''}</div>
 <div class="inspector-actions"><button class="primary" id="v83walk">🚶 Walk Here</button>${r.grade&&teacher?`<button class="primary" id="v83observe">👀 Observe Class</button><button class="secondary" id="v83talk">💬 Talk to ${teacher.name.split(' ')[0]}</button>`:''}<button class="secondary" id="inspectWO">Create Work Order</button>${r.grade?`<button class="secondary" id="inspectRoster">View Roster</button>`:''}</div>`;
 $('v83walk').onclick=()=>walkPrincipalTo(id);
 if($('v83observe'))$('v83observe').onclick=()=>v83Observe(teacher,id);
 if($('v83talk'))$('v83talk').onclick=()=>v911TalkEmployee(teacher);
 $('inspectWO').onclick=()=>openWorkOrderModal(id);
 if($('inspectRoster'))$('inspectRoster').onclick=()=>openRoster(id);
 box.querySelectorAll('.v911-room-talk').forEach(b=>b.onclick=()=>v911TalkEmployee(state.employees.find(x=>x.id===b.dataset.emp)));
}

function v911InterruptionMarkup(e){
 return `<p>${e.text}</p><p class="muted">Created ${e.time||mins12(state.simMinutes)} • ${e.status||'Pending'} • You are at ${locationLabel(state.principalLocation)}.</p><div class="actions">${(e.choices||[]).map((c,i)=>`<button class="${i===0?'primary':'secondary'} v911-event-choice" data-event-choice="${c}">${c}</button>`).join('')}<button class="secondary" id="v911Later">Handle Later</button></div>`;
}
function v911ResolveInterruption(e,c){
 e.status='In Progress';
 if((e.title==='Parent Waiting'&&c==='See parent now')||(e.title==='Office Call'&&c==='Take the call')){e.status='Handled';state.schoolHistory.unshift({date:state.date,text:`${e.title}: ${c}.`});v904ParentConversation(e.title);return;}
 if(e.title==='Classroom Support'&&c==='Go to classroom'){e.status='Handled';state.schoolHistory.unshift({date:state.date,text:`${e.title}: ${c}.`});v905ClassroomSupport(e);return;}
 if(e.title==='Facilities Alert'&&c==='Inspect it'){e.status='Handled';state.schoolHistory.unshift({date:state.date,text:`${e.title}: ${c}.`});v905FacilitiesInspection(e);return;}
 if(/now|call|classroom|inspect/i.test(c))state.simMinutes=Math.min(1080,state.simMinutes+20);
 if(/Schedule|follow-up|message/i.test(c))addTask(`${e.title}: ${e.text}`,'Today','Normal');
 if(c==='Send AP')addTask(`AP follow-up: ${e.text}`,'Today','Normal');
 if(c==='Delegate to head custodian')addTask(`Custodial follow-up: ${e.text}`,'Today','Normal');
 e.status='Handled';e.resolution=c;state.schoolHistory.unshift({date:state.date,text:`${e.title}: ${c}.`});closeModal();render();
}
function v911OpenInterruption(id){
 ensureV84State();let e=state.interruptions.find(x=>x.id===id);if(!e||e.status==='Handled')return;
 openModal(`📱 ${e.title}`,v911InterruptionMarkup(e));
 document.querySelectorAll('.v911-event-choice').forEach(b=>b.onclick=()=>v911ResolveInterruption(e,b.dataset.eventChoice));
 $('v911Later').onclick=()=>{e.status='Pending';closeModal();toast('Event kept in Pending Events.');render();};
}
maybeInterrupt=function(){
 ensureV84State();if(state.simMinutes<450||state.simMinutes>960||Math.random()>.018)return;
 let e=pick([
  {title:'Parent Waiting',text:'A parent arrived unexpectedly and is asking to speak with you.',choices:['See parent now','Ask AP to handle','Schedule meeting']},
  {title:'Office Call',text:'The secretary has a family on the phone asking specifically for the principal.',choices:['Take the call','Have secretary take a message','Delegate to AP']},
  {title:'Classroom Support',text:'A teacher is requesting brief administrative support with a recurring classroom concern.',choices:['Go to classroom','Send AP','Ask teacher to document and meet later']},
 ...
 if(e.title==='Classroom Support'){let teachers=activeEmployees().filter(x=>x.category==='Teacher'&&x.room);let t=pick(teachers);let kids=t?(state.students||[]).filter(x=>x.status==='Active'&&x.room===t.room):[];e.teacherId=t?.id||null;e.roomId=t?.room||null;e.studentId=pick(kids)?.id||null;e.issue=pick(['repeated off-task behavior','a peer conflict that is disrupting instruction','a student refusing to begin work','a student becoming increasingly upset','a recurring classroom-management concern']);if(t)e.text=`${t.name} is requesting brief administrative support in ${t.room}.`;}
 if(e.title==='Facilities Alert'){let room=pick((state.rooms||[]).filter(r=>r.type!=='circulation'));e.roomId=room?.id||null;e.issue=pick(['a ceiling tile that appears damp','an unusually warm room','a loose door closer','a sink that is draining slowly','a floor area that needs attention','a light fixture that is flickering']);if(room)e.text=`Custodial staff found an issue in ${room.id} — ${room.name} that may need your attention.`;}
 let rec={id:uid('int'),date:state.date,time:mins12(state.simMinutes),status:'Pending',...e};state.interruptions.unshift(rec);
 let modalOpen=!$('modal')?.classList.contains('hidden');
 if(modalOpen){toast(`New pending event: ${e.title}`);render();return;}
 v911OpenInterruption(rec.id);
}
function v911PendingEvents(){
 ensureV84State();let pending=state.interruptions.filter(x=>x.status!=='Handled');
 openModal(`🔔 Pending Events (${pending.length})`,`<div class="app-sheet"><p class="muted">Events stay here until you handle or delegate them. A new event will no longer erase the one you are working on.</p>${pending.map(x=>`<button class="pending-event-row" data-pending-event="${x.id}"><strong>${x.title}</strong><span>${x.time} • ${x.status||'Pending'}</span><small>${x.text}</small></button>`).join('')||'<p class="muted">Nothing is waiting for you.</p>'}</div>`);
 document.querySelectorAll('[data-pending-event]').forEach(b=>b.onclick=()=>v911OpenInterruption(b.dataset.pendingEvent));
}
const v91RenderV911Old=v91Render;
v91Render=function(){
 v91RenderV911Old();
 let box=$('v91PrincipalExperience');if(!box)return;
 let pending=state.interruptions.filter(x=>x.status!=='Handled').length;
 let head=box.querySelector('.section-head');
 if(head&&!$('v911PendingBtn')){let b=document.createElement('button');b.id='v911PendingBtn';b.className=pending?'primary':'secondary';b.textContent=`🔔 Pending Events (${pending})`;b.onclick=v911PendingEvents;head.appendChild(b);}
}
const v91PrincipalHubV911Old=v91PrincipalHub;
v91PrincipalHub=function(){
 v91PrincipalHubV911Old();
 let grid=document.querySelector('.principal-hub-grid');
 if(grid&&!$('v911HubPending')){let b=document.createElement('button');b.id='v911HubPending';b.textContent=`🔔 Pending Events (${state.interruptions.filter(x=>x.status!=='Handled').length})`;b.onclick=()=>{closeModal();v911PendingEvents();};grid.appendChild(b);}
}



/* ================= V9.2 RELATIONSHIPS, MEMORY & CONTINUITY ================= */
const V92_SAVE_KEY="lincolnElementarySimulatorRelationshipsMemoryV92";
function ensureV92State(){
 ensureV91State();state.version=9.2;state.v92=state.v92||{};let V=state.v92;
 V.relationships=V.relationships||{};V.familyRelationships=V.familyRelationships||{};
 V.followUps=Array.isArray(V.followUps)?V.followUps:[];V.secretaryNotes=Array.isArray(V.secretaryNotes)?V.secretaryNotes:[];
 V.ap=V.ap||{name:'Ms. Grant',focus:'Discipline / Attendance / Dismissal',handled:0,notes:[]};
 V.counselor=V.counselor||{name:'Ms. Brown',focus:'Student support / family needs',handled:0,notes:[]};
 V.classroomCulture=V.classroomCulture||{};V.arrival=V.arrival||{buses:[],cars:0,breakfast:0,lastBuilt:''};
 V.dismissal=V.dismissal||{buses:[],changes:[],lastBuilt:''};V.afterSchool=V.afterSchool||{clubs:[],staffLate:[],cleaning:[],secured:false};
 V.evaluations=Array.isArray(V.evaluations)?V.evaluations:[];V.hiring=V.hiring||{tours:[],references:[],onboarding:[]};
 V.calendar=Array.isArray(V.calendar)?V.calendar:[];V.testing=V.testing||{active:false,window:'Spring',rooms:[],devices:0,notes:[]};
 V.pto=V.pto||{meetings:[],proposals:[],relationship:'Working partnership'};V.comms=Array.isArray(V.comms)?V.comms:[];
 V.drills=Array.isArray(V.drills)?V.drills:[];V.emergencies=Array.isArray(V.emergencies)?V.emergencies:[];
 V.projects=Array.isArray(V.projects)?V.projects:[];V.summer=V.summer||{active:false,tasks:[],moves:[],deepClean:[],registration:0};
 V.generations=V.generations||{graduates:[],siblings:[],returningApplicants:[],legacyFamilies:[]};
 V.subImpact=Array.isArray(V.subImpact)?V.subImpact:[];V.studentLunch=Array.isArray(V.studentLunch)?V.studentLunch:[];
 V.recess=Array.isArray(V.recess)?V.recess:[];V.nightLogs=Array.isArray(V.nightLogs)?V.nightLogs:[];
 activeEmployees().forEach(e=>{
   V.relationships[e.id]=V.relationships[e.id]||{person:e.name,role:e.position,trust:'Developing',contacts:0,lastContact:null,promisesKept:0,promisesMissed:0,history:[]};
 });
 state.families.forEach(f=>{
   V.familyRelationships[f.id]=V.familyRelationships[f.id]||{family:f.name||`${f.lastName} Family`,contacts:0,lastContact:null,history:[],tone:'Typical'};
 });
 state.rooms.filter(r=>r.grade).forEach((r,i)=>{
   V.classroomCulture[r.id]=V.classroomCulture[r.id]||{style:pick(['Calm and structured','Warm and conversational','High-energy and collaborative','Quiet and independent','Project-centered','Routine-driven']),transition:pick(['Smooth','Usually smooth','Needs reminders']),studentVoice:pick(['High','Moderate','Growing']),notes:[]};
 });
 if(!V.calendar.length){
   V.calendar=[
    {month:'August',event:'Opening week / family welcome',type:'School Life'},
    {month:'September',event:'Fall family night',type:'Family'},
    {month:'October',event:'Conferences / fall festival',type:'Family'},
    {month:'November',event:'Veterans/community program',type:'Tradition'},
    {month:'December',event:'Winter concert',type:'Tradition'},
    {month:'January',event:'Midyear data / staffing review',type:'District'},
    {month:'February',event:'Book fair / PTO event',type:'PTO'},
    {month:'March',event:'State testing preparation',type:'Testing'},
    {month:'April',event:'State testing window',type:'Testing'},
    {month:'May',event:'Field day / sixth-grade walk',type:'Tradition'}
   ];
 }
}
function v92Rel(e){ensureV92State();return state.v92.relationships[e.id]||(state.v92.relationships[e.id]={person:e.name,role:e.position,trust:'Developing',contacts:0,lastContact:null,promisesKept:0,promisesMissed:0,history:[]});}
function v92Fam(f){ensureV92State();if(!f)return null;return state.v92.familyRelationships[f.id]||(state.v92.familyRelationships[f.id]={family:f.name||`${f.lastName} Family`,contacts:0,lastContact:null,history:[],tone:'Typical'});}
function v92RememberEmployee(e,text){
 let R=v92Rel(e);R.contacts++;R.lastContact=state.date;R.history.unshift({date:state.date,text});R.history=R.history.slice(0,25);
 if(R.promisesKept>=3)R.trust='Strong working relationship';else if(R.contacts>=4)R.trust='Established';else R.trust='Developing';
}
function v92RememberFamily(f,text){
 let R=v92Fam(f);if(!R)return;R.contacts++;R.lastContact=state.date;R.history.unshift({date:state.date,text});R.history=R.history.slice(0,25);
 if(R.contacts>=5)R.tone='Well known to Lincoln';else if(R.contacts>=2)R.tone='Familiar';
}
function v92CreatePromise(person,text,due='Today',personId=null,kind='staff'){
 ensureV92State();let x={id:uid('v92promise'),person,personId,kind,text,due,created:state.date,status:'Open'};state.v92.followUps.unshift(x);addTask(`Promise to ${person}: ${text}`,due,'High');return x;
}
function v92PromisePanel(){
 ensureV92State();let open=state.v92.followUps.filter(x=>x.status==='Open');
 openModal(`🤝 Follow-Through (${open.length})`,`<div class="app-sheet"><p class="muted">People remember what you said you would do. Completing these improves the history of that relationship.</p>${open.map(x=>`<div class="compact-item"><strong>${x.person}</strong><br>${x.text}<br><span class="muted">${x.due} • promised ${x.created}</span><br><button class="primary v92-keep" data-id="${x.id}">Complete Follow-Up</button> <button class="secondary v92-delay" data-id="${x.id}">Move to Tomorrow</button></div>`).join('')||'<p class="muted">No open promises.</p>'}</div>`);
 document.querySelectorAll('.v92-keep').forEach(b=>b.onclick=()=>{let x=state.v92.followUps.find(y=>y.id===b.dataset.id);if(!x)return;x.status='Kept';x.completed=state.date;state.simMinutes=Math.min(1080,state.simMinutes+8);if(x.kind==='staff'&&x.personId){let e=state.employees.find(y=>y.id===x.personId);if(e){let R=v92Rel(e);R.promisesKept++;v92RememberEmployee(e,`Principal followed through: ${x.text}`);}}if(x.kind==='family'&&x.personId){let f=state.families.find(y=>y.id===x.personId);if(f)v92RememberFamily(f,`Principal followed through: ${x.text}`);}state.schoolHistory.unshift({date:state.date,text:`Follow-through completed for ${x.person}: ${x.text}`});closeModal();render();toast('Promise kept and recorded.');});
 document.querySelectorAll('.v92-delay').forEach(b=>b.onclick=()=>{let x=state.v92.followUps.find(y=>y.id===b.dataset.id);if(x){x.due='Tomorrow';toast('Moved to tomorrow.');closeModal();render();}});
}
function v92EmployeeConversation(e){
 ensureV92State();if(!e)return;let R=v92Rel(e),prior=R.history[0]?.text||'No recent principal conversation on file.';
 let role=e.category,topic,detail;
 if(role==='Cafeteria'){topic=pick(['Lunch flow','Food/equipment','Student behavior at lunch','Staffing coverage']);detail=pick(['The serving line has been backing up when upper grades arrive.','One warmer is taking longer than usual to reach temperature.','A few student tables have needed repeated reminders.','Coverage is thin when someone calls off.']);}
 else if(role==='Specials'){topic=pick(['Transitions','Supplies/equipment','Student engagement','Schedule']);detail=pick(['Two classes are arriving late and cutting into instruction.','A supply/equipment item is becoming a problem.','One grade has been especially engaged in the current unit.','The rotation schedule creates a tight transition.']);}
 else if(role==='Operations'){topic=pick(['Building condition','Room setup','Cleaning priorities','Work orders']);detail=pick(['There is a small issue the team is monitoring before it becomes a work order.','A room setup request is affecting the cleaning route.','The team is prioritizing high-traffic areas today.','An open work order may need district support.']);}
 else if(role==='Student Services'){topic=pick(['Student support','Family concern','Attendance','Peer conflict']);detail=pick(['A student may need a coordinated team response.','A family situation may begin affecting attendance.','A student has had repeated social difficulty this week.','A pattern is emerging that the support team should watch.']);}
 else{topic=pick(['Student concern','Classroom routines','Schedule','Instruction']);detail=pick(['One student may need more support than the current classroom strategy is providing.','Transitions have been harder than usual this week.','A schedule issue is making planning difficult.','The class had a strong lesson and the teacher wants to share what worked.']);}
 state.principalLocation=e.room||currentEmployeeLocation(e)||'OFFICE';if(state.play)state.play.principalLoc=state.principalLocation;
 openModal(`💬 ${e.name} — ${topic}`,`<div class="app-sheet"><h3>${e.name}</h3><p>${e.position} • Relationship: <strong>${R.trust}</strong></p><div class="compact-item"><strong>Last thing you discussed</strong><br>${prior}</div><div class="app-section"><strong>Today</strong><p>${detail}</p></div><div class="actions"><button class="primary" data-v92talk="listen">Tell me more</button><button class="secondary" data-v92talk="promise">I’ll follow up on this</button><button class="secondary" data-v92talk="support">What support would help?</button><button class="secondary" data-v92talk="thanks">Thanks for telling me</button></div></div>`);
 document.querySelectorAll('[data-v92talk]').forEach(b=>b.onclick=()=>{let a=b.dataset.v92talk;if(a==='listen'){state.simMinutes+=6;v92RememberEmployee(e,`${topic}: Principal listened and gathered more context.`);openModal(`💬 ${e.name} — More Detail`,`<p>${pick(['They give you a specific example from today and explain what they have already tried.','They explain this has happened more than once and they wanted you aware before it grows.','They clarify that they are not asking you to solve it immediately—just to help remove one barrier.'])}</p><div class="actions"><button class="primary" id="v92PromiseNow">Promise follow-up</button><button class="secondary" id="v92CloseTalk">Thank them and close</button></div>`);$('v92PromiseNow').onclick=()=>{v92CreatePromise(e.name,topic,e.category==='Teacher'?'Today':'This week',e.id,'staff');v92RememberEmployee(e,`Principal promised follow-up on ${topic}.`);closeModal();render();};$('v92CloseTalk').onclick=()=>{closeModal();render();};return;}
 if(a==='promise'){v92CreatePromise(e.name,topic,'Today',e.id,'staff');v92RememberEmployee(e,`Principal promised follow-up on ${topic}.`);}
 else if(a==='support'){addTask(`Support ${e.name}: ${topic}`,'Today','Normal');v92RememberEmployee(e,`Principal asked what support would help with ${topic}.`);}
 else v92RememberEmployee(e,`Informal check-in about ${topic}.`);
 state.simMinutes+=5;closeModal();render();});
}
v911TalkEmployee=v92EmployeeConversation;

function v92FamilyHistoryModal(f,st){
 let R=v92Fam(f);openModal(`👪 ${R.family}`,`<h3>${R.tone}</h3><p>Principal/family contacts: ${R.contacts}</p>${R.history.map(x=>`<div class="compact-item"><strong>${x.date}</strong><br>${x.text}</div>`).join('')||'<p class="muted">No principal contact history yet.</p>'}${st?`<div class="compact-item"><strong>Current student</strong><br>${st.first} ${st.last} • Grade ${st.grade}</div>`:''}`);
}
function v92SecretaryBrief(){
 ensureV92State();let V=state.v92,openPromises=V.followUps.filter(x=>x.status==='Open');
 let next=(state.schedule||[])[0];let wait=state.interruptions.filter(x=>x.status!=='Handled').length;
 openModal('🗃️ Secretary Morning / Midday Brief',`<div class="app-sheet"><h3>${state.principal91.secretary.name}</h3><p>"Here’s what I think you need to know."</p>
 <div class="compact-item"><strong>${wait} pending principal event(s)</strong><br>${wait?'I kept them in your queue so nothing gets lost.':'Nothing waiting right now.'}</div>
 <div class="compact-item"><strong>${openPromises.length} promise(s) to follow up</strong><br>${openPromises.slice(0,3).map(x=>`${x.person}: ${x.text}`).join('<br>')||'You are caught up.'}</div>
 <div class="compact-item"><strong>Next calendar item</strong><br>${next?`${next.time} — ${next.item}`:'No scheduled item found.'}</div>
 <div class="actions"><button class="primary" id="v92SecPromises">Open Follow-Ups</button><button class="secondary" id="v92SecEvents">Pending Events</button></div></div>`);
 $('v92SecPromises').onclick=()=>{closeModal();v92PromisePanel()};$('v92SecEvents').onclick=()=>{closeModal();v911PendingEvents()};
}
function v92LeadershipAutonomy(){
 ensureV92State();let V=state.v92;if(Math.random()>.12)return;
 let x=pick(['ap','counselor']);
 if(x==='ap'){V.ap.handled++;let note=pick(['AP handled a routine discipline referral and returned the student to class.','AP followed up on an attendance/tardy pattern with the office.','AP supervised a dismissal issue without principal escalation.','AP checked a classroom after a teacher requested routine support.']);V.ap.notes.unshift({date:state.date,text:note});state.schoolHistory.unshift({date:state.date,text:note});}
 else{V.counselor.handled++;let note=pick(['Counselor resolved a peer conflict through a brief restorative conversation.','Counselor checked in with a student showing anxiety about school.','Counselor contacted a family about a student support concern.','Counselor facilitated a small-group support check-in.']);V.counselor.notes.unshift({date:state.date,text:note});}
}
function v92ClassroomCulture(id){
 ensureV92State();let r=roomById(id),c=state.v92.classroomCulture[id];if(!r||!c)return;
 let t=teacherForRoom(id);openModal(`🏫 Classroom Culture — ${id}`,`<h3>${t?.name||r.name}</h3><div class="inspector-grid"><div class="inspector-stat"><span>Feel</span><strong>${c.style}</strong></div><div class="inspector-stat"><span>Transitions</span><strong>${c.transition}</strong></div><div class="inspector-stat"><span>Student voice</span><strong>${c.studentVoice}</strong></div></div><p>${c.notes[0]||'Your understanding of this room will grow through walkthroughs and conversations.'}</p><div class="actions">${t?`<button class="primary" id="v92CultureTalk">Check in with ${t.name}</button><button class="secondary" id="v92CultureObs">Walkthrough</button>`:''}</div>`);
 if(t){$('v92CultureTalk').onclick=()=>{closeModal();v92EmployeeConversation(t)};$('v92CultureObs').onclick=()=>{closeModal();v83Observe(t,id)};}
}
function v92LunchWithStudents(){
 ensureV92State();let kids=state.students.filter(x=>x.status==='Active').sort(()=>Math.random()-.5).slice(0,4);state.principalLocation='CAF';state.simMinutes+=20;
 let comments=kids.map(k=>`${k.first}: “${pick(['Do principals have homework?','We are making a giant poster in class.','The pizza today is actually really good.','Can we have an extra recess someday?','My bus driver knows my grandma.','Do you live at the school?'])}”`);
 state.v92.studentLunch.unshift({date:state.date,students:kids.map(k=>k.id),comments});
 openModal('🍎 Lunch With Students',`<p>You sit at a cafeteria table instead of eating at your desk.</p>${comments.map(x=>`<div class="compact-item">${x}</div>`).join('')}<p class="muted">Twenty minutes passed. Students noticed that you were present at lunch.</p>`);
}
function v92Recess(){
 ensureV92State();let st=pick(state.students.filter(x=>x.status==='Active')),event=pick(['You join a kickball conversation and hear about a class project.','A duty teacher mentions one corner of the playground has needed extra supervision.','Two students have a minor disagreement that is resolved with coaching.','Several students run over just to say hello.']);state.principalLocation='PLAYGROUND';state.simMinutes+=12;state.v92.recess.unshift({date:state.date,text:event,studentId:st?.id});openModal('🛝 Recess Presence',`<p>${event}</p><div class="actions">${st?`<button class="secondary" id="v92RecessKid">Check in with ${st.first}</button>`:''}</div>`);if(st)$('v92RecessKid').onclick=()=>{st.timeline=st.timeline||[];st.timeline.unshift({date:state.date,text:'Principal checked in during recess.'});closeModal();render();};
}
function v92ArrivalBoard(){
 ensureV92State();let A=state.v92.arrival;if(A.lastBuilt!==state.date){A.lastBuilt=state.date;A.buses=Array.from({length:8},(_,i)=>({bus:10+i,eta:`${7+Math.floor((45+i*3)/60)}:${String((45+i*3)%60).padStart(2,'0')} AM`,status:i<3?'Arrived':'En route',students:rnd(32,54)}));A.cars=rnd(45,80);A.breakfast=rnd(70,115);}
 openModal('🚌 Morning Arrival',`<h3>Arrival Operations</h3><p>Car riders projected: ${A.cars} • Breakfast students: ${A.breakfast}</p>${A.buses.map(x=>`<div class="position-row"><span>Bus ${x.bus} • ${x.students} students</span><strong>${x.status}</strong></div>`).join('')}<div class="actions"><button class="primary" id="v92ArrivalPresence">Go to Bus/Car Line</button></div>`);$('v92ArrivalPresence').onclick=()=>{state.principalLocation='ENTRY';state.simMinutes+=15;A.buses.forEach(x=>x.status='Arrived');state.schoolHistory.unshift({date:state.date,text:'Principal supervised morning arrival.'});closeModal();render();};
}
function v92DismissalBoard(){
 ensureV92State();let D=state.v92.dismissal;if(D.lastBuilt!==state.date){D.lastBuilt=state.date;D.buses=Array.from({length:8},(_,i)=>({bus:10+i,status:i===5?'Late 7 min':'On time',loaded:false}));D.changes=Array.from({length:rnd(1,5)},()=>pick(['Bus to car rider','Car rider to bus','Daycare change','Parent pickup authorization check']));}
 openModal('🚸 Dismissal',`<h3>Live Dismissal Board</h3>${D.buses.map(x=>`<div class="position-row"><span>Bus ${x.bus}</span><strong>${x.loaded?'Loaded':x.status}</strong></div>`).join('')}<p>Pickup changes: ${D.changes.length}</p><div class="actions"><button class="primary" id="v92DismissGo">Supervise Dismissal</button></div>`);$('v92DismissGo').onclick=()=>{state.principalLocation='ENTRY';state.simMinutes+=20;D.buses.forEach(x=>x.loaded=true);state.schoolHistory.unshift({date:state.date,text:'Principal supervised dismissal and transportation loading.'});closeModal();render();};
}
function v92AfterSchool(){
 ensureV92State();let A=state.v92.afterSchool;A.clubs=pick([['Choir rehearsal','Robotics club'],['Student council','Homework club'],['No clubs scheduled'],['Art club','Running club']]);A.staffLate=activeEmployees().filter(e=>['Teacher','Specials'].includes(e.category)).sort(()=>Math.random()-.5).slice(0,rnd(3,8)).map(e=>e.name);A.cleaning=state.rooms.filter(r=>r.grade).slice(0,6).map(r=>`${r.id}: ${pick(['vacuumed / reset','trash removed','desks wiped','floor work underway'])}`);
 openModal('🌙 After-School Building',`<h3>${mins12(state.simMinutes)} — Students are mostly gone</h3><div class="compact-item"><strong>Activities</strong><br>${A.clubs.join('<br>')}</div><div class="compact-item"><strong>Teachers still here</strong><br>${A.staffLate.join(', ')||'Few staff remain'}</div><div class="compact-item"><strong>Custodial progress</strong><br>${A.cleaning.join('<br>')}</div><div class="actions"><button class="primary" id="v92Leave">Leave for the day</button></div>`);$('v92Leave').onclick=()=>{A.secured=true;state.v92.nightLogs.unshift({date:state.date,text:'Evening custodial shift continued after principal departure. Building secured at end of shift.'});closeModal();finishLivingDay();};
}
function v92SubImpact(){
 ensureV92State();let cov=(state.coverage||[]).filter(x=>x.status!=='Uncovered');if(!cov.length)return;
 cov.forEach(c=>{if(Math.random()<.22){let quality=pick(['Strong day — sub knew routines','Adequate coverage','Rough transitions — extra referrals','Excellent preferred-sub coverage']);state.v92.subImpact.unshift({date:state.date,assignment:c.teacher||c.employee||'Classroom',quality});if(/Rough/.test(quality)){state.dailyCounters.officeReferrals=(state.dailyCounters.officeReferrals||0)+1;}}});
}
function v92Evaluations(){
 ensureV92State();let teachers=activeEmployees().filter(e=>e.category==='Teacher');let e=pick(teachers);if(!e)return;let obs=(e.observations||[]);let evidence=obs.slice(0,3).map(o=>`${o.date}: ${o.type} — ${o.strength||'evidence collected'}`).join('<br>')||'No walkthrough evidence yet.';
 openModal(`📋 Evaluation File — ${e.name}`,`<h3>${e.position}</h3><div class="compact-item"><strong>Evidence</strong><br>${evidence}</div><div class="actions"><button class="primary" data-eval="conference">Hold evidence conference</button><button class="secondary" data-eval="walk">Schedule another walkthrough</button></div>`);
 document.querySelectorAll('[data-eval]').forEach(b=>b.onclick=()=>{if(b.dataset.eval==='conference'){state.v92.evaluations.unshift({date:state.date,employeeId:e.id,type:'Evidence conference',evidenceCount:obs.length});state.simMinutes+=25;v92RememberEmployee(e,'Evaluation evidence conference completed.');}else addTask(`Walkthrough for ${e.name}`,'This week','Normal');closeModal();render();});
}
function v92HiringExperience(){
 ensureV92State();let app=state.applications.find(a=>!['Hired','Rejected','Withdrawn'].includes(a.status));if(!app){toast('No active candidate file right now.');return;}
 openModal(`🤝 Candidate Experience — ${app.name}`,`<h3>${app.position}</h3><p>${app.experience} years experience • ${app.degree} • ${app.license}</p><div class="actions"><button class="primary" data-hirex="tour">Give building tour</button><button class="secondary" data-hirex="refs">Call references</button><button class="secondary" data-hirex="offer">Discuss offer</button></div>`);
 document.querySelectorAll('[data-hirex]').forEach(b=>b.onclick=()=>{let a=b.dataset.hirex;if(a==='tour'){state.v92.hiring.tours.unshift({date:state.date,candidate:app.name});app.status=app.status==='Applied'?'Interviewing':app.status;state.simMinutes+=20;}if(a==='refs'){state.v92.hiring.references.unshift({date:state.date,candidate:app.name,result:pick(['Strong recommendation','Positive with normal growth areas','Mixed feedback — follow up needed'])});state.simMinutes+=12;}if(a==='offer'){app.status='Offer Pending';state.simMinutes+=10;}closeModal();render();});
}
function v92Calendar(){
 ensureV92State();openModal('📅 Real School Calendar',`<div class="app-sheet">${state.v92.calendar.map(x=>`<div class="calendar-day"><strong>${x.month}</strong><span>${x.event}</span><small>${x.type}</small></div>`).join('')}</div>`);
}
function v92Testing(){
 ensureV92State();let T=state.v92.testing;T.active=true;T.rooms=state.rooms.filter(r=>r.grade).slice(0,9).map(r=>r.id);T.devices=Math.min(enrollment(),rnd(280,380));T.notes.unshift({date:state.date,text:'Testing schedule adjusted specials, devices, small-group rooms, and staff coverage.'});addTask('Finalize state testing room/coverage plan','This week','High');openModal('📝 Testing Season',`<p>Testing mode is active. ${T.rooms.length} rooms are reserved and ${T.devices} devices are assigned.</p><p>Specials schedules, small-group rooms, technology support, and coverage are affected.</p>`);
}
function v92PTO(){
 ensureV92State();let proposal={id:uid('pto'),date:state.date,item:pick(['Playground shade structure','Classroom mini-grants','Family literacy night','New recess equipment','Teacher appreciation week']),amount:rnd(800,6000),status:'Proposed'};state.v92.pto.proposals.unshift(proposal);
 openModal('👨‍👩‍👧 PTO Meeting',`<h3>Tonight's proposal</h3><p><strong>${proposal.item}</strong> — ${money(proposal.amount)}</p><p>PTO officers want the school’s input before committing funds.</p><div class="actions"><button class="primary" data-pto="support">Support proposal</button><button class="secondary" data-pto="revise">Ask for revision</button><button class="secondary" data-pto="decline">Do not support</button></div>`);
 document.querySelectorAll('[data-pto]').forEach(b=>b.onclick=()=>{proposal.status=b.dataset.pto==='support'?'Supported':b.dataset.pto==='revise'?'Revision Requested':'Not Supported';state.v92.pto.meetings.unshift({date:state.date,proposal:proposal.item,outcome:proposal.status});state.simMinutes+=30;closeModal();render();});
}
function v92Communication(){
 ensureV92State();openModal('📰 Lincoln Communications',`<form id="v92CommsForm"><div class="form-grid"><label>Channel<select id="v92Channel"><option>Family Newsletter</option><option>School Website</option><option>Social Update</option><option>Marquee</option></select></label><label class="full">Message<textarea id="v92Message" rows="4">A great week at Lincoln! Thank you to our students, staff, and families.</textarea></label></div><div class="actions"><button class="primary">Publish</button></div></form>`);
 $('v92CommsForm').onsubmit=e=>{e.preventDefault();let x={date:state.date,channel:$('v92Channel').value,message:$('v92Message').value.trim()};state.v92.comms.unshift(x);state.schoolHistory.unshift({date:state.date,text:`Published ${x.channel}: ${x.message}`});closeModal();render();};
}
function v92Drill(){
 ensureV92State();let type=pick(['Fire Drill','Tornado Drill','Lockdown Drill']);let mins=type==='Fire Drill'?12:18;state.simMinutes+=mins;state.v92.drills.unshift({date:state.date,time:mins12(state.simMinutes),type,result:pick(['Completed smoothly','One transition issue noted','Strong staff response'])});state.schoolHistory.unshift({date:state.date,text:`${type} completed.`});openModal(`🚨 ${type}`,`<p>The drill physically changed classroom activity and movement for about ${mins} minutes.</p><p><strong>${state.v92.drills[0].result}</strong></p>`);
}
function v92Emergency(){
 ensureV92State();let issue=pick(['Brief power outage','Water service problem','Severe thunderstorm warning','Ambulance requested for a medical emergency','Bus incident notification from transportation']);let x={date:state.date,time:mins12(state.simMinutes),issue,status:'Active'};state.v92.emergencies.unshift(x);
 openModal('⚠️ Rare Emergency',`<h3>${issue}</h3><p>This is uncommon and requires coordinated school/district response.</p><div class="actions"><button class="primary" data-em="respond">Activate response plan</button><button class="secondary" data-em="district">Contact district / emergency partners</button></div>`);
 document.querySelectorAll('[data-em]').forEach(b=>b.onclick=()=>{x.status='Managed';x.response=b.dataset.em;state.simMinutes+=20;state.schoolHistory.unshift({date:state.date,text:`Emergency response: ${issue} — ${x.response}.`});closeModal();render();});
}
function v92Project(){
 ensureV92State();let p={id:uid('proj'),name:pick(['Playground equipment replacement','Classroom furniture refresh','Library furniture project','HVAC controls upgrade','Front entrance safety improvement']),phase:'Requested',room:pick(state.rooms)?.id,budget:rnd(5000,55000),visible:false};state.v92.projects.unshift(p);
 openModal('🏗️ Building Project',`<h3>${p.name}</h3><p>${p.room} • Estimated ${money(p.budget)}</p><div class="actions"><button class="primary" data-proj="approve">Approve planning</button><button class="secondary" data-proj="hold">Hold</button></div>`);
 document.querySelectorAll('[data-proj]').forEach(b=>b.onclick=()=>{p.phase=b.dataset.proj==='approve'?'Ordered / Planning':'On Hold';if(p.phase!=='On Hold'){state.connected90.orders.unshift({id:uid('order'),item:p.name,cost:p.budget,status:'Ordered',ordered:state.date,room:p.room});}closeModal();render();});
}
function v92AdvanceProjects(){
 ensureV92State();state.v92.projects.forEach(p=>{if(p.phase==='Ordered / Planning'&&Math.random()<.22)p.phase='Delivery / Installation';else if(p.phase==='Delivery / Installation'&&Math.random()<.3){p.phase='Completed';p.visible=true;let r=roomById(p.room);if(r){r.equipment=r.equipment||[];r.equipment.push(p.name);r.condition=clamp(r.condition+3,0,100);}state.schoolHistory.unshift({date:state.date,text:`Building project completed: ${p.name}.`});}});
}
function v92Summer(){
 ensureV92State();let S=state.v92.summer;S.active=!S.active;if(S.active){S.tasks=['Staffing / vacancies','Classroom moves','Deep cleaning / floor work','Construction / repairs','Registration','Opening-day preparation'];S.moves=activeEmployees().filter(e=>e.category==='Teacher').sort(()=>Math.random()-.5).slice(0,3).map(e=>`${e.name}: room/grade review`);S.deepClean=state.rooms.filter(r=>r.grade).slice(0,8).map(r=>`${r.id}: scheduled`);S.registration=rnd(8,30);}openModal('☀️ Summer Operations',`<h3>${S.active?'Summer mode active':'Summer mode closed'}</h3>${S.active?`<p>${S.tasks.join(' • ')}</p><div class="compact-item"><strong>Potential classroom moves</strong><br>${S.moves.join('<br>')}</div><div class="compact-item"><strong>Pending registrations</strong><br>${S.registration}</div>`:'<p>Instructional-year operations remain active.</p>'}`);
}
function v92Generations(){
 ensureV92State();let G=state.v92.generations;let grads=state.students.filter(s=>s.status==='Promoted Out');G.graduates=grads.map(s=>({id:s.id,name:`${s.first} ${s.last}`,last:s.last,years:s.history?.length||1}));
 let famCounts={};state.students.forEach(s=>famCounts[s.familyId]=(famCounts[s.familyId]||0)+1);G.legacyFamilies=Object.entries(famCounts).filter(([,n])=>n>=2).map(([id,n])=>({family:state.families.find(f=>f.id===id)?.name||id,children:n}));
 openModal('🕰️ Lincoln Generations',`<div class="inspector-grid"><div class="inspector-stat"><span>Former students</span><strong>${G.graduates.length}</strong></div><div class="inspector-stat"><span>Multi-child families</span><strong>${G.legacyFamilies.length}</strong></div></div><h3>Legacy families</h3>${G.legacyFamilies.slice(0,8).map(x=>`<div class="compact-item"><strong>${x.family}</strong><br>${x.children} children connected to Lincoln records</div>`).join('')||'<p class="muted">Generational history will grow over time.</p>'}`);
}
function v92LifecycleYear(){
 ensureV92State();let G=state.v92.generations;
 state.students.filter(s=>s.status==='Promoted Out').forEach(s=>{if(Math.random()<.015&&!G.returningApplicants.some(x=>x.studentId===s.id)){G.returningApplicants.push({studentId:s.id,name:`${s.first} ${s.last}`,status:'Future educator interest',year:state.schoolYear});}});
 activeEmployees().forEach(e=>{if(e.life?.age>=62&&Math.random()<.12){e.life.retirementSignal='Considering retirement';}});
}
function v92Hub(){
 ensureV92State();let V=state.v92;
 openModal('🌳 Relationships, Memory & School Life',`<div class="principal-hub-grid">
 <button id="v92Follow">🤝 Follow-Through (${V.followUps.filter(x=>x.status==='Open').length})</button><button id="v92Sec">🗃️ Secretary Brief</button><button id="v92Lunch">🍎 Lunch With Students</button><button id="v92Recess">🛝 Recess Presence</button>
 <button id="v92Arrival">🚌 Morning Arrival</button><button id="v92Dismiss">🚸 Dismissal</button><button id="v92After">🌙 After School</button><button id="v92Eval">📋 Evaluations</button>
 <button id="v92Hire">🤝 Hiring Experience</button><button id="v92Cal">📅 School Calendar</button><button id="v92Test">📝 Testing Season</button><button id="v92PTO">👨‍👩‍👧 PTO Meeting</button>
 <button id="v92Comms">📰 Communications</button><button id="v92Drill">🚨 Drill</button><button id="v92Emergency">⚠️ Rare Emergency</button><button id="v92Project">🏗️ Building Project</button>
 <button id="v92Summer">☀️ Summer Operations</button><button id="v92Gen">🕰️ Generations</button></div>
 <h3>Leadership autonomy</h3><div class="compact-item"><strong>${V.ap.name}</strong> — ${V.ap.focus}<br>${V.ap.handled} routine items handled</div><div class="compact-item"><strong>${V.counselor.name}</strong> — ${V.counselor.focus}<br>${V.counselor.handled} student/family items handled</div>`);
 [['v92Follow',v92PromisePanel],['v92Sec',v92SecretaryBrief],['v92Lunch',v92LunchWithStudents],['v92Recess',v92Recess],['v92Arrival',v92ArrivalBoard],['v92Dismiss',v92DismissalBoard],['v92After',v92AfterSchool],['v92Eval',v92Evaluations],['v92Hire',v92HiringExperience],['v92Cal',v92Calendar],['v92Test',v92Testing],['v92PTO',v92PTO],['v92Comms',v92Communication],['v92Drill',v92Drill],['v92Emergency',v92Emergency],['v92Project',v92Project],['v92Summer',v92Summer],['v92Gen',v92Generations]].forEach(([id,fn])=>{if($(id))$(id).onclick=()=>{closeModal();fn();};});
}
function v92Render(){
 ensureV92State();let host=$('v91PrincipalExperience');if(!host)return;let box=$('v92Relationships');if(!box){box=document.createElement('section');box.id='v92Relationships';box.className='card padded';host.parentNode.insertBefore(box,host.nextSibling);}
 let V=state.v92,known=Object.values(V.relationships).filter(x=>x.contacts>0).length,open=V.followUps.filter(x=>x.status==='Open').length;
 box.innerHTML=`<div class="section-head"><div><h2>🌳 Relationships, Memory & Continuity</h2><p class="muted">Lincoln remembers people, promises, routines and years.</p></div><button class="primary" id="v92OpenHub">Open School Life</button></div>
 <div class="v91-strip"><button id="v92FollowQuick">🤝 Follow-Ups <strong>${open}</strong></button><button id="v92SecretaryQuick">🗃️ Secretary Brief</button><button id="v92ArrivalQuick">🚌 Arrival</button><button id="v92DismissQuick">🚸 Dismissal</button></div>
 <div class="v91-columns"><div><h3>Relationships</h3><div class="compact-item">Staff you have meaningful history with: <strong>${known}</strong></div><div class="compact-item">Families with principal contact history: <strong>${Object.values(V.familyRelationships).filter(x=>x.contacts>0).length}</strong></div></div>
 <div><h3>School Runs Without You</h3><div class="compact-item">AP routine items handled: <strong>${V.ap.handled}</strong></div><div class="compact-item">Counselor support items handled: <strong>${V.counselor.handled}</strong></div><div class="compact-item">After-hours logs: <strong>${V.nightLogs.length}</strong></div></div></div>`;
 $('v92OpenHub').onclick=v92Hub;$('v92FollowQuick').onclick=v92PromisePanel;$('v92SecretaryQuick').onclick=v92SecretaryBrief;$('v92ArrivalQuick').onclick=v92ArrivalBoard;$('v92DismissQuick').onclick=v92DismissalBoard;
}
const renderV92Old=render;render=function(){ensureV92State();renderV92Old();v92Render();};
const tickV92Old=playableTick;playableTick=function(minutes=5){tickV92Old(minutes);v92LeadershipAutonomy();v92AdvanceProjects();if(Math.random()<.015)v92SubImpact();};
const finishV92Old=finishLivingDay;finishLivingDay=function(){ensureV92State();state.v92.nightLogs.unshift({date:state.date,text:`Night shift: ${pick(['all classroom trash removed and rooms reset','floor work completed in primary hall','one room left for morning follow-up due to staffing','building secured after evening cleaning'])}.`});finishV92Old();};
const yearV92Old=endSchoolYear;endSchoolYear=function(){v92LifecycleYear();yearV92Old();ensureV92State();state.v92.summer.active=false;};
const saveV92Old=save;save=function(silent=false){ensureV92State();try{localStorage.setItem(V92_SAVE_KEY,JSON.stringify(state));}catch(e){console.error(e)}return saveV92Old(silent);};
const loadV92Old=load;load=function(){let raw=localStorage.getItem(V92_SAVE_KEY);if(raw){try{state=JSON.parse(raw);ensureV92State();toast('V9.2 Relationships & Memory save loaded.');render();return;}catch(e){console.error(e)}}return loadV92Old();};
const v91PrincipalHubV92Old=v91PrincipalHub;v91PrincipalHub=function(){v91PrincipalHubV92Old();let grid=document.querySelector('.principal-hub-grid');if(grid&&!$('v92HubLink')){let b=document.createElement('button');b.id='v92HubLink';b.textContent='🌳 Relationships & School Life';b.onclick=()=>{closeModal();v92Hub();};grid.appendChild(b);}};
const v83RoomInspectorV92Old=v83RoomInspector;v83RoomInspector=function(){v83RoomInspectorV92Old();let box=$('roomInspector'),id=state.selectedRoom;if(!box||!id)return;let r=roomById(id);if(r?.grade&&!$('v92CultureBtn')){let b=document.createElement('button');b.id='v92CultureBtn';b.className='secondary';b.textContent='🌱 Classroom Culture';b.onclick=()=>v92ClassroomCulture(id);box.querySelector('.inspector-actions')?.appendChild(b);}};



/* V9.2 memory integration with existing conversations and walkthroughs */
const v907FinishParentConversationV92Old=v907FinishParentConversation;
v907FinishParentConversation=function(ctx,choice){
 let fam=ctx?.fam,st=ctx?.st,topic=ctx?.topic?.kind||'family concern';
 v907FinishParentConversationV92Old(ctx,choice);
 if(fam)v92RememberFamily(fam,`Conversation regarding ${st?st.first+' '+st.last:'student'}: ${topic}. Principal response: ${choice}.`);
 if(fam&&choice==='investigate')v92CreatePromise(fam.name||`${fam.lastName} Family`,`Follow up regarding ${topic}`,'Today',fam.id,'family');
};
const observeTeacherV92Old=observeTeacher;
observeTeacher=function(e,formal=false){
 let before=(state.observations||[]).length;
 let result=observeTeacherV92Old(e,formal);
 ensureV92State();
 let newest=(state.observations||[])[0];
 if(e&&newest&&(state.observations||[]).length>=before){
   let C=state.v92.classroomCulture[e.room];
   if(C){C.notes.unshift(`${state.date}: ${formal?'Formal observation':'Walkthrough'} — ${newest.strength||'evidence collected'}${newest.growth?` Growth: ${newest.growth}`:''}`);C.notes=C.notes.slice(0,12);}
   v92RememberEmployee(e,`${formal?'Formal observation':'Walkthrough'} completed in ${e.room}.`);
 }
 return result;
};
const v92TickDeepOld=playableTick;
playableTick=function(minutes=5){
 v92TickDeepOld(minutes);
 ensureV92State();
 if(Math.random()<.00025&&!$('modal')?.classList.contains('hidden')===false){
   let issue=pick(['Brief utility disruption','Severe weather bulletin','Unexpected medical response','Transportation incident notification']);
   state.v92.emergencies.unshift({date:state.date,time:mins12(state.simMinutes),issue,status:'Reported'});
   addLiveAlert('⚠️','Rare operational event',issue,'Urgent','OFFICE');
 }
};



/* V9.3 — School Leadership & Staffing: persistent, actionable management */
const V93_SAVE_KEY='lincolnElementarySimulatorLeadershipV93';
function v93Init(){
 ensureV92State();state.v93=state.v93||{};let M=state.v93;
 for(let k of ['decisions','staffMeetings','assignmentHistory','leaveRequests','vacancyRequests','departmentChanges','staffNotices','annualPlans','coverageHistory','onboarding','managementLog'])if(!Array.isArray(M[k]))M[k]=[];
 M.preferences=M.preferences||{};M.departments=M.departments||{};M.nextYear=M.nextYear||{};M.stage=M.stage||'Current year';
 for(let e of activeEmployees()){
  if(!M.preferences[e.id])M.preferences[e.id]={grades:e.category==='Teacher'?[String(e.assignment||'').replace('Grade ','')]:[],request:'Stay in current assignment',status:'Not discussed'};
  if(!M.nextYear[e.id])M.nextYear[e.id]={room:e.room,assignment:e.assignment,fte:e.fte};
 }
 for(let cat of ['Cafeteria','Operations','Specials','Office','Student Services','Special Education','Instructional Support','Technology'])if(!M.departments[cat])M.departments[cat]={lead:activeEmployees().find(e=>e.category===cat)?.id||'',focus:'Normal operations',duties:{}};
 if(M.year!==state.schoolYear){M.year=state.schoolYear;M.stage='Opening / staffing';M.annualPlans.unshift({year:state.schoolYear,date:state.date,summary:'New school year staffing and opening review'});}
}
function v93Log(text){v93Init();state.v93.managementLog.unshift({date:state.date,time:mins12(state.simMinutes),text});state.v93.managementLog=state.v93.managementLog.slice(0,100);state.schoolHistory.unshift({date:state.date,text});}
function v93Spend(n=5){state.simMinutes=Math.min(1080,state.simMinutes+n);}
function v93RoomOptions(gradeOnly=false){return state.rooms.filter(r=>gradeOnly?!!r.grade:!!r.id).map(r=>`<option value="${r.id}">${r.id} — ${r.name}${r.grade?' (Grade '+r.grade+')':''}</option>`).join('');}
function v93StaffOptions(cat='',selected=''){return activeEmployees().filter(e=>!cat||e.category===cat).map(e=>`<option value="${e.id}" ${e.id===selected?'selected':''}>${e.name} — ${e.position}</option>`).join('');}
function v93Board(){v93Init();let M=state.v93;
 openModal('📋 Lincoln — Staffing & Assignment Board',`<div class="v93modal"><p>Move or swap teachers, plan next year, review vacancies and class sizes. Current-year changes affect the live map and students; next-year plans remain proposals until finalized.</p><div class="v93toolbar"><button class="primary" id="v93Move">↔ Reassign / Swap</button><button class="secondary" id="v93Plan">📅 Next-Year Plan</button><button class="secondary" id="v93Vac">➕ Position Request</button><button class="secondary" id="v93Staff">👥 Staff Profiles</button></div><div class="v93roomgrid">${GRADES.map(g=>`<section><h3>Grade ${g}</h3>${state.rooms.filter(r=>String(r.grade)===String(g)).map(r=>{let t=teacherForRoom(r.id),n=studentsInRoom(r.id).length,planned=t&&M.nextYear[t.id];return `<button class="v93room" data-v93room="${r.id}"><strong>${r.id} — ${t?.name||'VACANT'}</strong><span>${n} students / ${r.capacity} seats ${n>r.capacity?'⚠️':''}</span><small>${t?`${t.experience} years • ${t.fte} FTE • next: ${planned?.room||r.id}`:'Recruit or transfer teacher'}</small></button>`}).join('')}</section>`).join('')}</div><h3>Recent assignment decisions</h3>${M.assignmentHistory.slice(0,5).map(x=>`<div class="compact-item">${x.date}: ${x.text}</div>`).join('')||'<p class="muted">No changes yet.</p>'}</div>`);
 $('v93Move').onclick=v93Move;$('v93Plan').onclick=v93Plan;$('v93Vac').onclick=v93Position;$('v93Staff').onclick=v93Profiles;document.querySelectorAll('[data-v93room]').forEach(b=>b.onclick=()=>v93Room(b.dataset.v93room));}
function v93Room(id){let r=roomById(id),t=teacherForRoom(id);if(!r)return;openModal(`🏫 Room ${id} — Assignment`, `<p>${r.name} • Grade ${r.grade} • ${studentsInRoom(id).length}/${r.capacity} students</p><p><strong>${t?.name||'Vacant'}</strong>${t?` • ${t.experience} years • ${t.fte} FTE`:''}</p><div class="actions"><button class="primary" id="v93RoomMove">Change teacher assignment</button>${t?'<button class="secondary" id="v93RoomMeet">Meet with teacher</button>':''}<button class="secondary" id="v93RoomBoard">Back to board</button></div>`);$('v93RoomMove').onclick=()=>v93Move(id);if(t)$('v93RoomMeet').onclick=()=>v93Meet(t.id);$('v93RoomBoard').onclick=v93Board;}
function v93Move(target=''){v93Init();openModal('↔ Teacher Reassignment / Swap',`<form id="v93MoveForm" class="v93form"><label>Teacher<select id="v93Teacher" required>${v93StaffOptions('Teacher')}</select></label><label>Destination classroom<select id="v93Target" required>${v93RoomOptions(true)}</select></label><label>Effective date<select id="v93Effective"><option value="now">Current school year — change now</option><option value="next">Next school year — proposed</option></select></label><label>Reason<textarea id="v93Reason" rows="2" required>Enrollment and instructional staffing needs</textarea></label><p class="muted" id="v93MovePreview">Review assignment before confirming.</p><button class="primary">Review consequences</button></form>`);if(target)$('v93Target').value=target;
 function preview(){let e=state.employees.find(x=>x.id===$('v93Teacher').value),r=roomById($('v93Target').value),other=teacherForRoom(r?.id);$('v93MovePreview').textContent=`${e?.name||'Teacher'}: ${e?.room||'unassigned'} → ${r?.id||'?'} (Grade ${r?.grade||'?'}). ${other&&other.id!==e?.id?'Occupied by '+other.name+'; a swap is required.':'Old classroom will become vacant unless filled.'} ${$('v93Effective').value==='next'?'Plan only; current classrooms stay unchanged.':'Changes current staffing and generates HR/communication tasks.'}`;}
 for(let id of ['v93Teacher','v93Target','v93Effective'])$(id).onchange=preview;preview();$('v93MoveForm').onsubmit=ev=>{ev.preventDefault();let id=$('v93Teacher').value,dest=$('v93Target').value,when=$('v93Effective').value,reason=$('v93Reason').value.trim(),e=state.employees.find(x=>x.id===id),r=roomById(dest),other=teacherForRoom(dest);if(!e||!r?.grade||!reason)return;if(e.room===dest){toast('Teacher already assigned to that room.');return;}if(other&&other.id!==id){v93ConfirmMove(id,dest,when,reason,true);}else v93ConfirmMove(id,dest,when,reason,false);};}
function v93ConfirmMove(id,dest,when,reason,swap){let e=state.employees.find(x=>x.id===id),other=teacherForRoom(dest),old=e?.room,r=roomById(dest);if(!e||!r)return;openModal('⚠️ Review Staffing Decision',`<h3>${e.name}: ${old} → ${dest}</h3><p>${swap?`This is a <strong>swap</strong> with ${other.name}. Both teachers will change rooms and grade assignments.`:'The former classroom will have a vacancy until another teacher is assigned.'}</p><p>Reason: ${reason}</p><p>${when==='now'?'Current map and teacher records update immediately. Student rosters stay in their classrooms. HR, family communication, curriculum preparation and room-move follow-ups are generated.':'This is a next-year plan only. Current room assignments and rosters remain unchanged until you finalize the plan.'}</p><div class="actions"><button class="primary" id="v93Commit">Confirm ${when==='now'?'Current Assignment':'Next-Year Plan'}</button><button class="secondary" id="v93Cancel">Cancel</button></div>`);$('v93Cancel').onclick=()=>v93Move(dest);$('v93Commit').onclick=()=>{v93Init();let M=state.v93;if(when==='next'){M.nextYear[id]={room:dest,assignment:`Grade ${r.grade}`,fte:e.fte};if(swap&&other)M.nextYear[other.id]={room:old,assignment:`Grade ${roomById(old)?.grade||'Unassigned'}`,fte:other.fte};}else{let oldGrade=roomById(old)?.grade;if(swap&&other){other.room=old;other.assignment=`Grade ${oldGrade||'Unassigned'}`;other.position=`Grade ${oldGrade||'Unassigned'} Teacher`;other.history=other.history||[];other.history.unshift(`${state.date}: Swapped to ${old} from ${dest}.`);M.nextYear[other.id]={room:old,assignment:other.assignment,fte:other.fte};}e.room=dest;e.assignment=`Grade ${r.grade}`;e.position=`Grade ${r.grade} Teacher`;e.history=e.history||[];e.history.unshift(`${state.date}: Assignment changed ${old} → ${dest}.`);M.nextYear[id]={room:dest,assignment:e.assignment,fte:e.fte};addTask(`Notify HR and families about assignment change: ${e.name}`,'Today','High');addTask(`Arrange room materials and curriculum transition: ${old} → ${dest}`,'This week','Normal');if(!swap)addTask(`Fill teaching vacancy in room ${old}`,'This week','High');}let text=`${e.name}: ${old} → ${dest}${swap?' (swap with '+other.name+')':''} (${when==='next'?'next-year proposal':'effective now'}). ${reason}`;M.assignmentHistory.unshift({date:state.date,text,employeeId:id,from:old,to:dest,effective:when});v93Log(text);v93Spend(12);closeModal();render();v93Board();};}
function v93Profiles(){v93Init();openModal('👥 Employee Assignment & Intent Profiles',`<div class="v93modal"><p>Discuss preferences before moving staff. These are conversations and records, not automatic consent.</p>${activeEmployees().map(e=>`<button class="v93room" data-profile="${e.id}"><strong>${e.name} — ${e.position}</strong><span>${e.assignment} • ${e.room} • ${e.fte} FTE</span><small>${state.v93.preferences[e.id]?.request||'No request'} • ${state.v93.preferences[e.id]?.status||'Not discussed'}</small></button>`).join('')}</div>`);document.querySelectorAll('[data-profile]').forEach(b=>b.onclick=()=>v93Meet(b.dataset.profile));}
function v93Meet(id){v93Init();let e=state.employees.find(x=>x.id===id);if(!e)return;let pref=state.v93.preferences[id]||{request:'Stay in current assignment',status:'Not discussed'};openModal(`💬 Assignment Meeting — ${e.name}`,`<form id="v93MeetForm" class="v93form"><p>${e.position} • ${e.assignment} • ${e.room} • ${e.experience} years • ${e.contract||'Contract on file'}</p><p>Prior relationship: ${state.v92.relationships[id]?.trust||'Developing'}</p><label>Teacher/staff request<select id="v93Request"><option>Stay in current assignment</option><option>Request different grade</option><option>Request different building</option><option>Reduced FTE / job share interest</option><option>Leave request</option><option>Considering retirement</option><option>Interested in leadership</option></select></label><label>Preferred grade (teachers)<select id="v93Grade"><option value="">No preference</option>${GRADES.map(g=>`<option value="${g}">${g}</option>`).join('')}</select></label><label>Meeting notes<textarea id="v93Notes" rows="3" required>Discussed staffing preferences and school needs.</textarea></label><button class="primary">Record Meeting</button></form>`);$('v93Request').value=pref.request||'Stay in current assignment';$('v93Grade').value=pref.grades?.[0]||'';$('v93MeetForm').onsubmit=ev=>{ev.preventDefault();let req=$('v93Request').value,grade=$('v93Grade').value,notes=$('v93Notes').value.trim();if(!notes)return;state.v93.preferences[id]={request:req,grades:grade?[grade]:[],status:'Discussed',notes};state.v93.staffMeetings.unshift({date:state.date,employeeId:id,request:req,grade,notes});e.history=e.history||[];e.history.unshift(`${state.date}: Assignment meeting — ${req}. ${notes}`);if(req==='Leave request')state.v93.leaveRequests.unshift({id:uid('leave93'),employeeId:id,type:'Requested leave',date:state.date,status:'Pending HR review'});if(req==='Considering retirement')addTask(`Discuss succession and retirement timeline with ${e.name}`,'This month','Normal');v93Log(`Assignment meeting with ${e.name}: ${req}`);v93Spend(20);closeModal();render();v93Profiles();};}
function v93Plan(){v93Init();let M=state.v93;openModal('📅 Next-Year Staffing Plan',`<div class="v93modal"><p>Proposals do not change the current school day. Review the planned assignments, then finalize them at the appropriate year transition.</p>${GRADES.map(g=>`<h3>Grade ${g}</h3>${state.rooms.filter(r=>String(r.grade)===String(g)).map(r=>{let planned=activeEmployees().filter(e=>e.category==='Teacher').find(e=>M.nextYear[e.id]?.room===r.id);return `<div class="position-row"><span>${r.id} • ${studentsInRoom(r.id).length} current students</span><strong>${planned?.name||'VACANT'}</strong></div>`}).join('')}`).join('')}<div class="actions"><button class="primary" id="v93Finalize">Finalize planned assignments</button><button class="secondary" id="v93EditPlan">Edit assignments</button></div></div>`);$('v93EditPlan').onclick=()=>v93Move();$('v93Finalize').onclick=()=>v93Finalize();}
function v93Finalize(){v93Init();let teachers=activeEmployees().filter(e=>e.category==='Teacher'),plans=state.v93.nextYear,ids=teachers.map(e=>plans[e.id]?.room).filter(Boolean),duplicates=ids.filter((x,i)=>ids.indexOf(x)!==i);if(duplicates.length){openModal('⚠️ Resolve Assignment Conflicts',`<p>Two or more teachers are planned for ${[...new Set(duplicates)].join(', ')}. Resolve these conflicts before finalizing.</p><button class="primary" id="v93Fix">Back to board</button>`);$('v93Fix').onclick=v93Board;return;}openModal('Confirm Annual Staffing Plan',`<p>This applies the proposed assignments to current teacher records. Existing students stay assigned to their classrooms. Any unfilled rooms remain vacancies.</p><div class="actions"><button class="primary" id="v93FinalYes">Finalize and notify staff</button><button class="secondary" id="v93FinalNo">Not yet</button></div>`);$('v93FinalNo').onclick=v93Plan;$('v93FinalYes').onclick=()=>{for(let e of teachers){let p=plans[e.id];if(p&&p.room&&e.room!==p.room){let old=e.room;e.room=p.room;e.assignment=p.assignment||`Grade ${roomById(p.room)?.grade}`;e.position=`${e.assignment} Teacher`;e.history=e.history||[];e.history.unshift(`${state.date}: Annual assignment finalized: ${old} → ${p.room}`);}}state.v93.stage='Assignments finalized';v93Log('Annual teacher assignment plan finalized; notifications and classroom moves required.');addTask('Send annual teacher assignment letters','This week','High');addTask('Coordinate classroom moves and curriculum materials','This week','Normal');v93Spend(30);closeModal();render();v93Board();};}
function v93Position(){v93Init();openModal('➕ Request an Authorized Position',`<form id="v93PositionForm" class="v93form"><p>Building principals request positions; district HR/finance must approve authorization before a new posting is created.</p><label>Position title<input id="v93Title" required value="Elementary Classroom Teacher"></label><label>Department<select id="v93Category">${[...new Set(activeEmployees().map(e=>e.category))].map(x=>`<option>${x}</option>`).join('')}</select></label><label>FTE<input id="v93Fte" type="number" min="0.2" max="1" step="0.2" value="1" required></label><label>Justification<textarea id="v93Justification" required rows="3">Projected enrollment and classroom capacity</textarea></label><button class="primary">Submit to District</button></form>`);$('v93Category').value='Teacher';$('v93PositionForm').onsubmit=ev=>{ev.preventDefault();let x={id:uid('pos93'),date:state.date,title:$('v93Title').value.trim(),category:$('v93Category').value,fte:Number($('v93Fte').value),reason:$('v93Justification').value.trim(),status:'District review'};if(!x.title||!x.reason||x.fte<=0)return;state.v93.vacancyRequests.unshift(x);v93Log(`Position requested: ${x.title} (${x.fte} FTE), pending district review.`);addTask(`Follow up with district HR: ${x.title} position request`,'This week','Normal');v93Spend(15);closeModal();render();v93HR();};}
function v93ApprovePosition(id,approved){let x=state.v93.vacancyRequests.find(y=>y.id===id);if(!x||x.status!=='District review')return;x.status=approved?'Authorized':'Not authorized';if(approved){let p=state.positions.find(p=>p.role===x.title||p.category===x.category&&p.role===x.title);if(p)p.authorized+=x.fte;else state.positions.push({id:uid('position93'),role:x.title,category:x.category,fte:x.fte,authorized:x.fte,filled:0});addTask(`Post authorized vacancy: ${x.title}`,'This week','High');}v93Log(`District ${approved?'authorized':'declined'} position request: ${x.title}.`);render();v93HR();}
function v93HR(){v93Init();let M=state.v93;openModal('🗂️ Human Resources — Principal Office',`<div class="v93modal"><div class="v93toolbar"><button class="primary" id="v93HRReq">Request Position</button><button class="secondary" id="v93HRProfiles">Assignment Meetings</button><button class="secondary" id="v93HRPipeline">Candidate Interviews</button><button class="secondary" id="v93HRLeave">Leave / Coverage</button></div><h3>District position control</h3>${M.vacancyRequests.map(x=>`<div class="compact-item"><strong>${x.title}</strong> • ${x.fte} FTE<br>${x.reason}<br><span class="muted">${x.status}</span>${x.status==='District review'?`<br><button class="secondary" data-authorize="${x.id}">Record District Approval</button><button class="secondary" data-decline="${x.id}">Record Denial</button>`:''}</div>`).join('')||'<p class="muted">No new position requests.</p>'}<h3>Staff notices / leave</h3>${M.leaveRequests.slice(0,8).map(x=>`<div class="compact-item">${state.employees.find(e=>e.id===x.employeeId)?.name||'Employee'} • ${x.type} • ${x.status}</div>`).join('')||'<p class="muted">No pending leave requests.</p>'}<h3>Hiring & onboarding</h3>${M.onboarding.slice(0,5).map(x=>`<div class="compact-item">${x.name} • ${x.status} • ${x.position}</div>`).join('')||'<p class="muted">No onboarding cases yet.</p>'}</div>`);$('v93HRReq').onclick=v93Position;$('v93HRProfiles').onclick=v93Profiles;$('v93HRPipeline').onclick=v93Candidates;$('v93HRLeave').onclick=v93Coverage;document.querySelectorAll('[data-authorize]').forEach(b=>b.onclick=()=>v93ApprovePosition(b.dataset.authorize,true));document.querySelectorAll('[data-decline]').forEach(b=>b.onclick=()=>v93ApprovePosition(b.dataset.decline,false));}
function v93Candidates(){let apps=state.applications.filter(a=>!['Hired','Rejected','Withdrawn'].includes(a.status));openModal('🤝 Recruiting & Candidate Decisions',`<div class="v93modal"><p>Interview, references, tours, offer and onboarding remain separate steps. District HR confirms hiring authorization.</p>${apps.map(a=>`<button class="v93room" data-candidate="${a.id}"><strong>${a.name||'Candidate'} — ${a.position||'Open position'}</strong><span>${a.status}</span></button>`).join('')||'<p class="muted">No active candidates. Open HR to request or post a position.</p>'}</div>`);document.querySelectorAll('[data-candidate]').forEach(b=>b.onclick=()=>v93Candidate(b.dataset.candidate));}
function v93Candidate(id){let a=state.applications.find(x=>x.id===id);if(!a)return;openModal(`Candidate — ${a.name}`,`<p>${a.position||'Position'} • ${a.status}</p><p>Experience: ${a.experience||'Not listed'} • Degree: ${a.degree||'Not listed'} • License: ${a.license||'Verify with HR'}</p><div class="actions"><button class="primary" data-stage="Interview Scheduled">Schedule Interview</button><button class="secondary" data-stage="References Checked">Call References</button><button class="secondary" data-stage="Tour Completed">Give Building Tour</button><button class="secondary" data-stage="Offer Pending">Request Offer Approval</button><button class="secondary" data-stage="Onboarding">Record Accepted Offer / Begin Onboarding</button><button class="secondary" data-stage="Withdrawn">Candidate Withdraws</button></div>`);document.querySelectorAll('[data-stage]').forEach(b=>b.onclick=()=>{let stage=b.dataset.stage;a.status=stage;v93Log(`${a.name}: ${stage}.`);v93Spend(stage==='Interview Scheduled'?5:stage==='Tour Completed'?25:12);if(stage==='Onboarding'){state.v93.onboarding.unshift({id:uid('onboard93'),name:a.name,position:a.position||'Staff',status:'Pending background clearance, HR forms, badge, mentor and start date'});addTask(`Complete onboarding and clearance for ${a.name}`,'This week','High');}closeModal();render();v93Candidates();});}
function v93Coverage(){v93Init();openModal('📆 Leave, Coverage & Staff Scheduling',`<div class="v93modal"><div class="v93toolbar"><button class="primary" id="v93NewLeave">Record Leave Request</button><button class="secondary" id="v93Dept">Department Assignments</button></div><h3>Absences and coverage</h3>${state.absences.map(a=>`<div class="compact-item">${state.employees.find(e=>e.id===a.employeeId)?.name||a.name||'Employee'} • ${a.reason||'Absent'} • ${state.coverage.find(c=>c.absenceId===a.id)?.status||'Coverage needed'}</div>`).join('')||'<p class="muted">No absences currently recorded.</p>'}<h3>Pending leave</h3>${state.v93.leaveRequests.filter(x=>x.status==='Pending HR review').map(x=>`<div class="compact-item"><strong>${state.employees.find(e=>e.id===x.employeeId)?.name||'Staff'}</strong> • ${x.type}<br><button class="secondary" data-leave-approve="${x.id}">Approve subject to HR</button><button class="secondary" data-leave-decline="${x.id}">Return for discussion</button></div>`).join('')||'<p class="muted">No pending requests.</p>'}</div>`);$('v93NewLeave').onclick=v93Leave;$('v93Dept').onclick=v93Departments;document.querySelectorAll('[data-leave-approve]').forEach(b=>b.onclick=()=>v93LeaveDecision(b.dataset.leaveApprove,true));document.querySelectorAll('[data-leave-decline]').forEach(b=>b.onclick=()=>v93LeaveDecision(b.dataset.leaveDecline,false));}
function v93Leave(){openModal('Leave / FTE Request',`<form class="v93form" id="v93LeaveForm"><label>Employee<select id="v93LeaveEmp">${v93StaffOptions()}</select></label><label>Request<select id="v93LeaveType"><option>Personal / PTO</option><option>Medical leave</option><option>Parental leave</option><option>Reduced FTE</option><option>Long-term absence</option></select></label><label>Dates / details<textarea id="v93LeaveNotes" rows="3" required></textarea></label><button class="primary">Record for HR Review</button></form>`);$('v93LeaveForm').onsubmit=ev=>{ev.preventDefault();let id=$('v93LeaveEmp').value,x={id:uid('leave93'),employeeId:id,type:$('v93LeaveType').value,notes:$('v93LeaveNotes').value.trim(),date:state.date,status:'Pending HR review'};if(!x.notes)return;state.v93.leaveRequests.unshift(x);v93Log(`Leave request submitted: ${state.employees.find(e=>e.id===id)?.name} — ${x.type}.`);closeModal();render();v93Coverage();};}
function v93LeaveDecision(id,yes){let x=state.v93.leaveRequests.find(y=>y.id===id);if(!x)return;x.status=yes?'Principal approved — HR confirmation needed':'Returned for discussion';if(yes)addTask(`Arrange coverage and confirm HR leave: ${state.employees.find(e=>e.id===x.employeeId)?.name}`,'Today','High');v93Log(`${x.type}: ${x.status}.`);render();v93Coverage();}
function v93Departments(){v93Init();openModal('🏢 Department Management',`<div class="v93modal"><p>Set responsibilities, shifts and priorities for cooks, custodians, specials, office, nurse/counselor, paras and technology.</p>${Object.entries(state.v93.departments).map(([cat,d])=>`<button class="v93room" data-dept="${cat}"><strong>${cat}</strong><span>${activeEmployees().filter(e=>e.category===cat).length} employees • ${d.focus}</span><small>${Object.keys(d.duties).length} duty assignments</small></button>`).join('')}</div>`);document.querySelectorAll('[data-dept]').forEach(b=>b.onclick=()=>v93Department(b.dataset.dept));}
function v93Department(cat){let M=state.v93,d=M.departments[cat],staff=activeEmployees().filter(e=>e.category===cat);let duties=cat==='Cafeteria'?['Breakfast prep','Serving line','Cashier / POS','Dish room','Lunch cleanup','Inventory']:cat==='Operations'?['Opening inspection','Classroom cleaning','Restrooms','Floor care','Waste / recycling','Closing inspection']:cat==='Specials'?['Morning specials','Afternoon specials','Arrival duty','Lunch duty','Dismissal duty','Equipment / inventory']:cat==='Office'?['Attendance','Visitor check-in','Parent calls','Substitute check-in','Dismissal changes']:cat==='Student Services'?['Nurse triage','Counseling','MTSS meetings','Attendance follow-up','Family outreach']:['Instructional support','Student supervision','Planning / preparation','Coverage','Equipment'];openModal(`🏢 ${cat} — Staff Assignments`, `<div class="v93modal"><p>Department priority: <strong>${d.focus}</strong></p><div class="v93toolbar"><button class="primary" id="v93DeptFocus">Change Priority</button><button class="secondary" id="v93DeptBack">All Departments</button></div>${staff.map(e=>`<div class="compact-item"><strong>${e.name}</strong> — ${e.position}<br>${e.schedule} • ${e.fte} FTE • ${e.room}<br>Duty: ${d.duties[e.id]||'Regular assignment'}<br><button class="secondary" data-duty="${e.id}">Change Duty / Schedule</button><button class="secondary" data-talk="${e.id}">Check In</button></div>`).join('')}</div>`);$('v93DeptBack').onclick=v93Departments;$('v93DeptFocus').onclick=()=>{openModal(`${cat} — Department Priorities`,`<form id="v93FocusForm" class="v93form"><label>Priority<select id="v93Focus"><option>Normal operations</option><option>Coverage / staffing</option><option>Student experience</option><option>Safety and compliance</option><option>Inventory / equipment</option><option>Schedule redesign</option></select></label><button class="primary">Assign Priority</button></form>`);$('v93FocusForm').onsubmit=ev=>{ev.preventDefault();d.focus=$('v93Focus').value;v93Log(`${cat} department priority: ${d.focus}.`);addTask(`Review ${cat.toLowerCase()} priority: ${d.focus}`,'This week','Normal');closeModal();render();v93Department(cat);};};document.querySelectorAll('[data-duty]').forEach(b=>b.onclick=()=>v93Duty(cat,b.dataset.duty,duties));document.querySelectorAll('[data-talk]').forEach(b=>b.onclick=()=>v92EmployeeConversation(state.employees.find(e=>e.id===b.dataset.talk)));}
function v93Duty(cat,id,duties){let e=state.employees.find(x=>x.id===id);if(!e)return;openModal(`🕒 Assign ${e.name}`,`<form id="v93DutyForm" class="v93form"><label>Duty<select id="v93Duty">${duties.map(x=>`<option>${x}</option>`).join('')}</select></label><label>Schedule<input id="v93Schedule" value="${e.schedule||''}" required></label><label>Location<select id="v93Location">${v93RoomOptions()}</select></label><label>FTE<input id="v93DutyFte" type="number" min="0.2" max="1" step="0.2" value="${e.fte}" required></label><p class="muted">FTE changes create an HR approval request; schedules/duties change now. Check contract and required coverage.</p><button class="primary">Save Assignment</button></form>`);$('v93Duty').value=state.v93.departments[cat].duties[id]||duties[0];$('v93Location').value=e.room;$('v93DutyForm').onsubmit=ev=>{ev.preventDefault();let duty=$('v93Duty').value,schedule=$('v93Schedule').value.trim(),room=$('v93Location').value,fte=Number($('v93DutyFte').value);if(!schedule||!Number.isFinite(fte)||fte<.2||fte>1)return;let old=e.room;if(fte!==e.fte){state.v93.leaveRequests.unshift({id:uid('fte93'),employeeId:id,type:`FTE change ${e.fte} → ${fte}`,notes:'Principal-requested assignment adjustment',date:state.date,status:'Pending HR review'});addTask(`Confirm HR approval for ${e.name} FTE change`,'This week','High');}e.schedule=schedule;e.room=room;state.v93.departments[cat].duties[id]=duty;v93Log(`${e.name}: ${duty}, ${schedule}, ${old} → ${room}. FTE request ${fte}.`);v93Spend(8);closeModal();render();v93Department(cat);};}
function v93Agenda(){v93Init();let M=state.v93,teacherVac=state.rooms.filter(r=>r.grade&&!teacherForRoom(r.id)),uncovered=state.coverage.filter(c=>c.status==='Uncovered'),leave=M.leaveRequests.filter(x=>x.status==='Pending HR review'),requests=M.vacancyRequests.filter(x=>x.status==='District review'),promises=state.v92.followUps.filter(x=>x.status==='Open');let agenda=[{title:`Review ${teacherVac.length} vacant classrooms`,fn:v93Board,why:'Assignment and section coverage'},{title:`Resolve ${uncovered.length} uncovered absences`,fn:v93Coverage,why:'Daily staffing and substitutes'},{title:`Review ${leave.length} leave/FTE requests`,fn:v93Coverage,why:'Staff scheduling and HR'},{title:`Follow up on ${requests.length} district position requests`,fn:v93HR,why:'Authorization and budget'},{title:'Meet with teachers about next-year assignments',fn:v93Profiles,why:'Preferences, retention and transfers'},{title:`Complete ${promises.length} staff/family follow-ups`,fn:v92PromisePanel,why:'Relationships and commitments'},{title:'Inspect department assignments',fn:v93Departments,why:'Cafeteria, custodial, specials and support'},{title:'Review annual staffing plan',fn:v93Plan,why:'Next-year organization'}];openModal('🗓️ Principal Management Agenda',`<div class="v93modal"><p>${fmtDate(state.date)} • ${state.schoolYear}. Choose a decision to work on; agenda items open their actual management workflow.</p>${agenda.map((a,i)=>`<button class="v93room" data-agenda="${i}"><strong>${a.title}</strong><span>${a.why}</span></button>`).join('')}</div>`);document.querySelectorAll('[data-agenda]').forEach(b=>b.onclick=()=>agenda[Number(b.dataset.agenda)].fn());}
function v93Year(){v93Init();let m=Number(String(state.date).slice(5,7)),focus=m===8?'Opening / schedules':m===9?'Enrollment and coverage':m<=12&&m>=10?'Observations and support':m<=2?'Budget and enrollment':m<=4?'Intent forms and recruiting':m<=6?'Final assignments and retirements':'Summer hiring and room preparation';let stages=[['August','Opening, schedules, rooms'],['September','Enrollment and coverage'],['October–December','Observations, evaluation, support'],['January–February','Budget and enrollment projections'],['March–April','Intent forms, transfers, recruiting'],['May–June','Assignments, retirements, moves'],['July','Hiring, deep cleaning, opening preparation']];openModal('📆 Annual Leadership Cycle',`<p><strong>Current focus: ${focus}</strong></p>${stages.map(x=>`<div class="position-row"><strong>${x[0]}</strong><span>${x[1]}</span></div>`).join('')}<div class="actions"><button class="primary" id="v93YearPlan">Open Next-Year Plan</button><button class="secondary" id="v93YearAgenda">Today’s Management Agenda</button></div>`);$('v93YearPlan').onclick=v93Plan;$('v93YearAgenda').onclick=v93Agenda;}
function v93Hub(){v93Init();let M=state.v93;openModal('🏫 School Leadership — Principal Management',`<div class="v93modal"><h3>Run Lincoln: assignments, people, schedules and resources</h3><p>${state.schoolYear} • ${activeEmployees().length} active staff • ${state.rooms.filter(r=>r.grade&&!teacherForRoom(r.id)).length} unstaffed classrooms</p><div class="principal-hub-grid"><button id="v93HubBoard">📋 Staffing Board</button><button id="v93HubAgenda">🗓️ Management Agenda</button><button id="v93HubProfiles">👥 Staff Meetings</button><button id="v93HubDepts">🏢 Departments</button><button id="v93HubHR">🗂️ HR Office</button><button id="v93HubCoverage">📆 Leave & Coverage</button><button id="v93HubCandidates">🤝 Hiring Pipeline</button><button id="v93HubPlan">📅 Next-Year Assignments</button><button id="v93HubYear">🔁 Annual Cycle</button><button id="v93HubPosition">➕ Position Request</button></div><h3>Recent management decisions</h3>${M.managementLog.slice(0,6).map(x=>`<div class="compact-item">${x.date} • ${x.text}</div>`).join('')||'<p class="muted">Start by reviewing your staffing board.</p>'}</div>`);let links={v93HubBoard:v93Board,v93HubAgenda:v93Agenda,v93HubProfiles:v93Profiles,v93HubDepts:v93Departments,v93HubHR:v93HR,v93HubCoverage:v93Coverage,v93HubCandidates:v93Candidates,v93HubPlan:v93Plan,v93HubYear:v93Year,v93HubPosition:v93Position};Object.entries(links).forEach(([id,fn])=>$(id).onclick=fn);}
function v93Render(){v93Init();let host=$('v92Relationships')||$('v91PrincipalExperience');if(!host)return;let box=$('v93Management');if(!box){box=document.createElement('section');box.id='v93Management';box.className='card padded';host.parentNode.insertBefore(box,host);}let M=state.v93,open=state.rooms.filter(r=>r.grade&&!teacherForRoom(r.id)).length,leave=M.leaveRequests.filter(x=>x.status==='Pending HR review').length,req=M.vacancyRequests.filter(x=>x.status==='District review').length;box.innerHTML=`<div class="section-head"><div><h2>🏫 School Leadership & Staffing</h2><p class="muted">Make staffing decisions, run departments and plan the next school year.</p></div><button class="primary" id="v93Open">Open Principal Management</button></div><div class="v91-strip"><button id="v93BoardQuick">📋 Staffing Board <strong>${open} vacancies</strong></button><button id="v93AgendaQuick">🗓️ Agenda</button><button id="v93DeptQuick">🏢 Departments</button><button id="v93HRQuick">🗂️ HR <strong>${leave+req} pending</strong></button></div><p class="muted">${M.managementLog[0]?.text||'No staffing decisions recorded yet.'}</p>`;$('v93Open').onclick=v93Hub;$('v93BoardQuick').onclick=v93Board;$('v93AgendaQuick').onclick=v93Agenda;$('v93DeptQuick').onclick=v93Departments;$('v93HRQuick').onclick=v93HR;}
const v93RenderOld=render;render=function(){v93Init();v93RenderOld();v93Render();};
const v93SaveOld=save;save=function(silent=false){v93Init();try{localStorage.setItem(V93_SAVE_KEY,JSON.stringify(state));}catch(e){console.error(e)}return v93SaveOld(silent);};
const v93LoadOld=load;load=function(){let raw=localStorage.getItem(V93_SAVE_KEY);if(raw){try{state=JSON.parse(raw);v93Init();toast('V9.3 school management save loaded.');render();return;}catch(e){console.error(e)}}return v93LoadOld();};
const v93PrincipalHubOld=v91PrincipalHub;v91PrincipalHub=function(){v93PrincipalHubOld();let grid=document.querySelector('.principal-hub-grid');if(grid&&!$('v93PrincipalLink')){let b=document.createElement('button');b.id='v93PrincipalLink';b.textContent='🏫 School Leadership & Staffing';b.onclick=()=>{closeModal();v93Hub();};grid.prepend(b);}};
const v93YearOld=endSchoolYear;endSchoolYear=function(){v93Init();v93YearOld();v93Init();state.v93.stage='Opening / staffing';};


/* V9.4 District management: persistent staffing season, career progression, enrollment,
   coaching, recruiting competition, district approvals, physical moves, principal office,
   improvement plan and principal career. */
const V94_SAVE_KEY='lincolnElementarySimulatorDistrictV94';
function v94Init(){v93Init();let M=state.v94=state.v94||{};
 for(let k of ['placement','enrollmentHistory','coaching','careerEvents','districtRequests','roomMoves','recruitment','districtMeetings','officeMail','officeDesk','officeCalendar','schoolReviews','careerApplications','careerHistory','summerOperations'])if(!Array.isArray(M[k]))M[k]=[];
 M.staffCareers=M.staffCareers||{};M.sectionPlan=M.sectionPlan||{};M.projections=M.projections||{};M.strategy=M.strategy||{year:state.schoolYear,focus:'Building and facilities',goals:[],reviews:[]};M.district=M.district||{superintendent:'Dr. Morgan Ellis',hr:'District Human Resources',positionsAuthorized:0};M.career=M.career||{role:'Lincoln Elementary Principal',school:'Lincoln Elementary',tenure:1};M.staffLocations=M.staffLocations||{};
 activeEmployees().forEach(e=>{M.staffCareers[e.id]=M.staffCareers[e.id]||{employeeId:e.id,started:state.schoolYear,preferredGrades:[],goal:'Continue current assignment',certifications:[e.license||'Verify credentials'],mentorId:null,leadershipRole:'',retirement:'Not indicated',careerHistory:[]};});
 if(!M.projections[state.schoolYear]){let counts={};GRADES.forEach(g=>counts[g]=state.students.filter(s=>s.status==='Active'&&String(s.grade)===String(g)).length);M.projections[state.schoolYear]={...counts};}
}
function v94Record(text){v94Init();state.v94.careerHistory.unshift({date:state.date,text});state.schoolHistory.unshift({date:state.date,text});}
function v94StaffingSeason(){v94Init();let M=state.v94;openModal('🌷 Spring Teacher Placement & Intent Season',`<div class="v93modal"><p>Collect staff intentions, review projected enrollment, hold meetings, and prepare assignments without moving anyone during the current year.</p><div class="v93toolbar"><button id="v94Collect" class="primary">Collect Intent Forms</button><button id="v94Enroll" class="secondary">Enrollment & Sections</button><button id="v94Plan" class="secondary">Proposed Assignments</button></div>${activeEmployees().filter(e=>e.category==='Teacher').map(e=>{let x=M.placement.find(y=>y.employeeId===e.id);return `<div class="compact-item"><strong>${e.name}</strong> • ${e.room} • ${e.experience} years<br>${x?`${x.intent} • preferred ${x.grade||'current grade'} • ${x.status}`:'Intent not submitted'}<br><button data-intent="${e.id}" class="secondary">Review intent / meet</button></div>`}).join('')}</div>`);$('v94Collect').onclick=()=>{activeEmployees().filter(e=>e.category==='Teacher').forEach(e=>{if(!M.placement.some(x=>x.employeeId===e.id))M.placement.push({employeeId:e.id,date:state.date,intent:pick(['Remain in assignment','Request grade change','Interested in leadership','Considering transfer','Considering retirement']),grade:String(roomById(e.room)?.grade||''),status:'Submitted'});});v94Record('Teacher intent forms collected; individual meetings still required.');v94StaffingSeason();};$('v94Enroll').onclick=v94Enrollment;$('v94Plan').onclick=v93Plan;document.querySelectorAll('[data-intent]').forEach(b=>b.onclick=()=>v94Intent(b.dataset.intent));}
function v94Intent(id){let e=state.employees.find(x=>x.id===id);if(!e)return;let M=state.v94,x=M.placement.find(y=>y.employeeId===id);if(!x){x={employeeId:id,date:state.date,intent:'Remain in assignment',grade:String(roomById(e.room)?.grade||''),status:'Not discussed'};M.placement.push(x)}openModal(`Placement Meeting — ${e.name}`,`<form id="v94IntentForm" class="v93form"><p>Current: ${e.room} • ${e.assignment}. Meeting records do not automatically move an employee.</p><label>Intent<select id="v94IntentType">${['Remain in assignment','Request grade change','Interested in leadership','Considering transfer','Considering retirement','Reduced schedule interest'].map(z=>`<option>${z}</option>`).join('')}</select></label><label>Preferred grade<select id="v94IntentGrade"><option value="">No preference</option>${GRADES.map(g=>`<option value="${g}">${g}</option>`).join('')}</select></label><label>Meeting notes<textarea id="v94IntentNotes" required rows="3">Discussed next-year placement and student needs.</textarea></label><button class="primary">Record discussion</button></form>`);$('v94IntentType').value=x.intent;$('v94IntentGrade').value=x.grade||'';$('v94IntentForm').onsubmit=ev=>{ev.preventDefault();x.intent=$('v94IntentType').value;x.grade=$('v94IntentGrade').value;x.notes=$('v94IntentNotes').value.trim();if(!x.notes)return;x.status='Met with principal';x.date=state.date;M.staffCareers[id].preferredGrades=x.grade?[x.grade]:[];M.staffCareers[id].retirement=x.intent==='Considering retirement'?'Considering retirement':'Not indicated';v92RememberEmployee(e,`Annual placement meeting: ${x.intent}.`);v94Record(`Placement meeting with ${e.name}: ${x.intent}`);closeModal();v94StaffingSeason();};}
function v94Enrollment(){v94Init();let M=state.v94,project=M.projections[state.schoolYear],rooms=state.rooms.filter(r=>r.grade);openModal('📊 Enrollment-Driven Staffing & Sections',`<div class="v93modal"><p>Enter realistic enrollment projections. The planner compares available classroom seats and projected sections; it does not invent authorized staff positions.</p><form id="v94EnrollForm" class="v93form">${GRADES.map(g=>{let rr=rooms.filter(r=>String(r.grade)===String(g)),capacity=rr.reduce((n,r)=>n+(r.capacity||24),0),count=state.students.filter(s=>s.status==='Active'&&String(s.grade)===String(g)).length;return `<label>Grade ${g} — ${rr.length} rooms / ${capacity} seats / ${count} current students<input type="number" min="0" max="250" id="v94Count${g}" value="${project[g]??count}"></label>`}).join('')}<button class="primary">Save Projections & Calculate Needs</button></form><div class="v93toolbar"><button class="secondary" id="v94Growth">Simulate New Neighborhood Enrollment</button><button class="secondary" id="v94Sections">Review Section Decisions</button></div></div>`);$('v94EnrollForm').onsubmit=ev=>{ev.preventDefault();GRADES.forEach(g=>project[g]=Math.max(0,Number($('v94Count'+g).value)||0));M.enrollmentHistory.unshift({date:state.date,counts:{...project}});v94Record('Enrollment projections revised for staffing and space planning.');v94Sections();};$('v94Growth').onclick=()=>{let g=pick(GRADES);project[g]+=rnd(5,14);M.enrollmentHistory.unshift({date:state.date,change:`New subdivision projected + students in grade ${g}`,counts:{...project}});v94Sections();};$('v94Sections').onclick=v94Sections;}
function v94Sections(){v94Init();let M=state.v94,project=M.projections[state.schoolYear];openModal('🏫 Section & Capacity Decisions',`<div class="v93modal">${GRADES.map(g=>{let rr=state.rooms.filter(r=>String(r.grade)===String(g)),count=project[g]||0,cap=rr.reduce((a,r)=>a+(r.capacity||24),0),need=Math.ceil(count/24),staff=rr.filter(r=>teacherForRoom(r.id)).length;return `<div class="compact-item"><strong>Grade ${g}</strong> • ${count} projected students<br>${rr.length} physical classrooms • ${cap} seats • ${staff} assigned teachers • ${need} sections at 24 students<br><strong>${count>cap?'SPACE SHORTAGE':need>staff?'Staffing review needed':'Within planned capacity'}</strong><br><button class="secondary" data-section="${g}">Make section decision</button></div>`}).join('')}</div>`);document.querySelectorAll('[data-section]').forEach(b=>b.onclick=()=>v94SectionDecision(b.dataset.section));}
function v94SectionDecision(g){let M=state.v94,rr=state.rooms.filter(r=>String(r.grade)===String(g)),count=M.projections[state.schoolYear][g]||0;openModal(`Grade ${g} — Section Decision`,`<p>${count} projected students, ${rr.length} available rooms. Review district policy and room capacity before changes.</p><div class="actions"><button data-choice="Redistribute students" class="secondary">Redistribute within grade</button><button data-choice="Request additional teacher" class="primary">Request teacher authorization</button><button data-choice="Request classroom space" class="secondary">Request additional space</button><button data-choice="Monitor enrollment" class="secondary">Monitor projections</button></div>`);document.querySelectorAll('[data-choice]').forEach(b=>b.onclick=()=>{let choice=b.dataset.choice;M.sectionPlan[g]={choice,date:state.date,projection:count};if(choice.startsWith('Request'))v94DistrictRequest(choice==='Request additional teacher'?'Additional teacher':'Additional classroom space',`Grade ${g}: ${count} projected students across ${rr.length} rooms.`);else{v94Record(`Grade ${g} section plan: ${choice}.`);v94Sections();}});}
function v94Careers(){v94Init();openModal('👥 Employee Careers & Succession',`<div class="v93modal">${activeEmployees().map(e=>{let c=state.v94.staffCareers[e.id];return `<button class="v93room" data-career="${e.id}"><strong>${e.name} — ${e.position}</strong><span>${e.experience} years • Goal: ${c.goal}</span><small>${c.leadershipRole||'No leadership assignment'} • ${c.retirement}</small></button>`}).join('')}</div>`);document.querySelectorAll('[data-career]').forEach(b=>b.onclick=()=>v94Career(b.dataset.career));}
function v94Career(id){let e=state.employees.find(x=>x.id===id),c=state.v94.staffCareers[id];if(!e||!c)return;openModal(`Career File — ${e.name}`,`<form id="v94CareerForm" class="v93form"><p>${e.position} • ${e.experience} years experience • ${e.degree||'Degree not recorded'} • ${e.license||'Licensure to verify'}</p><label>Professional goal<select id="v94Goal">${['Continue current assignment','Change grade or department','Become a mentor','Lead a grade-level team','Pursue assistant principal role','Pursue principal role','Pursue head custodian / department lead','Prepare for retirement'].map(z=>`<option>${z}</option>`).join('')}</select></label><label>Leadership role<select id="v94Lead">${['','Mentor teacher','Grade-level lead','Department lead','Head custodian','School improvement team'].map(z=>`<option>${z}</option>`).join('')}</select></label><label>Mentor<select id="v94Mentor"><option value="">None</option>${activeEmployees().filter(x=>x.id!==id).map(x=>`<option value="${x.id}">${x.name}</option>`).join('')}</select></label><button class="primary">Save Career Plan</button></form><h3>Career history</h3>${c.careerHistory.slice(0,5).map(x=>`<div class="compact-item">${x}</div>`).join('')||'<p>No milestones recorded.</p>'}`);$('v94Goal').value=c.goal;$('v94Lead').value=c.leadershipRole;$('v94Mentor').value=c.mentorId||'';$('v94CareerForm').onsubmit=ev=>{ev.preventDefault();c.goal=$('v94Goal').value;c.leadershipRole=$('v94Lead').value;c.mentorId=$('v94Mentor').value||null;c.careerHistory.unshift(`${state.date}: Career plan updated — ${c.goal}; role ${c.leadershipRole||'none'}.`);if(c.mentorId)addTask(`Schedule mentoring check-in for ${e.name}`,'This month','Normal');v92RememberEmployee(e,`Career planning: ${c.goal}`);closeModal();v94Careers();};}
function v94Coaching(){v94Init();openModal('📚 Instructional Coaching & Evidence',`<div class="v93modal"><p>Support is gradual. Schedule a coaching cycle, document evidence, then review after multiple school days.</p>${activeEmployees().filter(e=>e.category==='Teacher').map(e=>{let c=state.v94.coaching.find(x=>x.employeeId===e.id&&x.status==='Active');return `<div class="compact-item"><strong>${e.name}</strong> • ${e.room}<br>Walkthroughs: ${(e.observations||[]).length} • ${c?`${c.focus}, ${c.days} days into cycle`:'No active coaching cycle'}<br><button class="secondary" data-coach="${e.id}">${c?'Review coaching':'Start coaching cycle'}</button></div>`}).join('')}</div>`);document.querySelectorAll('[data-coach]').forEach(b=>b.onclick=()=>v94Coach(b.dataset.coach));}
function v94Coach(id){let e=state.employees.find(x=>x.id===id),M=state.v94,c=M.coaching.find(x=>x.employeeId===id&&x.status==='Active');if(!e)return;if(c){openModal(`Coaching Review — ${e.name}`,`<p>${c.focus} • ${c.days} instructional days • ${c.checkIns} check-ins</p><p>Evidence: ${(e.observations||[]).slice(0,3).map(o=>o.strength||o.type).join('; ')||'Gather walkthrough evidence before concluding.'}</p><div class="actions"><button id="v94CoachCheck" class="primary">Hold coaching check-in</button><button id="v94CoachClose" class="secondary">Complete cycle (after 10 days)</button></div>`);$('v94CoachCheck').onclick=()=>{c.checkIns++;c.notes.push(`${state.date}: Coaching check-in and next steps.`);v92RememberEmployee(e,`Coaching check-in: ${c.focus}`);v94Coach(id);};$('v94CoachClose').onclick=()=>{if(c.days<10){toast('Coaching needs at least ten instructional days.');return;}c.status='Completed';c.completed=state.date;c.outcome=c.checkIns>=2?'Support and evidence reviewed':'Cycle completed; additional check-ins recommended';v94Record(`${e.name} completed coaching cycle: ${c.focus}.`);v94Coaching();};return;}
 openModal(`Start Coaching — ${e.name}`,`<form id="v94CoachForm" class="v93form"><label>Focus<select id="v94CoachFocus"><option>Classroom routines</option><option>Transitions</option><option>Student engagement</option><option>Differentiation</option><option>Assessment practices</option><option>Family communication</option></select></label><label>Support<select id="v94CoachSupport"><option>Mentor pairing</option><option>Instructional coach visits</option><option>Professional development</option><option>Collaborative planning</option></select></label><button class="primary">Begin 10-day coaching cycle</button></form>`);$('v94CoachForm').onsubmit=ev=>{ev.preventDefault();M.coaching.unshift({employeeId:id,focus:$('v94CoachFocus').value,support:$('v94CoachSupport').value,start:state.date,days:0,checkIns:0,status:'Active',notes:[]});addTask(`Check in on coaching: ${e.name}`,'This week','Normal');closeModal();v94Coaching();};}
function v94Recruiting(){v94Init();let M=state.v94;state.applications.filter(a=>!['Hired','Withdrawn','Rejected','Offer Declined'].includes(a.status)).forEach(a=>{if(!M.recruitment.some(x=>x.id===a.id))M.recruitment.push({id:a.id,otherOffers:rnd(0,2),daysWaiting:0,preferredGrade:pick(GRADES),salaryExpectation:rnd(42000,74000),startDate:'August',withdrawn:false});});openModal('🤝 Hiring Competition & Candidate Market',`<div class="v93modal">${M.recruitment.map(x=>{let a=state.applications.find(z=>z.id===x.id);return a?`<div class="compact-item"><strong>${a.name}</strong> — ${a.status}<br>Other offers: ${x.otherOffers} • Salary expectation: ${money(x.salaryExpectation)} • Grade preference: ${x.preferredGrade}<br>Waiting: ${x.daysWaiting} days • Start: ${x.startDate}<br><button class="secondary" data-recruit="${x.id}">Review candidate</button></div>`:''}).join('')||'<p>No active candidate files. Post a vacancy through HR first.</p>'}</div>`);document.querySelectorAll('[data-recruit]').forEach(b=>b.onclick=()=>v94RecruitCandidate(b.dataset.recruit));}
function v94RecruitCandidate(id){let a=state.applications.find(x=>x.id===id),x=state.v94.recruitment.find(y=>y.id===id);if(!a||!x)return;openModal(`Candidate Market — ${a.name}`,`<p>${a.position} • ${a.status}</p><p>${x.otherOffers} competing offer(s). Salary expectation ${money(x.salaryExpectation)}. Preferred grade ${x.preferredGrade}. Waiting ${x.daysWaiting} days.</p><div class="actions"><button id="v94RecruitInterview" class="primary">Schedule interview</button><button id="v94RecruitTour" class="secondary">Invite to school tour</button><button id="v94RecruitOffer" class="secondary">Request district-approved offer</button></div>`);$('v94RecruitInterview').onclick=()=>{a.status='Interview Scheduled';x.daysWaiting=0;v94Record(`Interview scheduled: ${a.name}.`);v94Recruiting();};$('v94RecruitTour').onclick=()=>{a.status='Tour Completed';x.daysWaiting=0;v94Record(`${a.name} toured Lincoln.`);v94Recruiting();};$('v94RecruitOffer').onclick=()=>v94DistrictRequest('Candidate offer approval',`${a.name}: ${a.position}, requested salary ${money(x.salaryExpectation)}.`,id);}
function v94DistrictRequest(type,reason,candidateId=''){v94Init();let x={id:uid('district94'),type,reason,candidateId,status:'Awaiting district decision',date:state.date};state.v94.districtRequests.unshift(x);addTask(`District review: ${type}`,'This week','High');v94Record(`Submitted district request: ${type}.`);v94District();}
function v94District(){v94Init();let M=state.v94;openModal('🏢 Superintendent & District HR',`<div class="v93modal"><p>${M.district.superintendent} • ${M.district.hr}. District decisions are separate from principal requests.</p><div class="v93toolbar"><button id="v94DistrictMeet" class="primary">Schedule Superintendent Meeting</button><button id="v94DistrictNew" class="secondary">New District Request</button></div>${M.districtRequests.map(x=>`<div class="compact-item"><strong>${x.type}</strong> • ${x.status}<br>${x.reason}<br>${x.status==='Awaiting district decision'?`<button class="secondary" data-district-approve="${x.id}">Record district approval</button><button class="secondary" data-district-deny="${x.id}">Record district denial</button>`:''}</div>`).join('')||'<p>No pending district requests.</p>'}</div>`);$('v94DistrictMeet').onclick=()=>{M.districtMeetings.unshift({date:state.date,topic:'Staffing, budget, enrollment and school improvement',status:'Scheduled'});addTask('Prepare superintendent meeting materials','This week','High');v94Record('Superintendent meeting scheduled.');v94District();};$('v94DistrictNew').onclick=()=>{openModal('Request District Action',`<form id="v94DistrictForm" class="v93form"><label>Type<select id="v94ReqType"><option>Additional teacher</option><option>Transfer authorization</option><option>Major purchase</option><option>Capital project</option><option>Contract / FTE adjustment</option></select></label><label>Justification<textarea id="v94ReqReason" required rows="3"></textarea></label><button class="primary">Submit Request</button></form>`);$('v94DistrictForm').onsubmit=ev=>{ev.preventDefault();let reason=$('v94ReqReason').value.trim();if(reason)v94DistrictRequest($('v94ReqType').value,reason);};};document.querySelectorAll('[data-district-approve],[data-district-deny]').forEach(b=>b.onclick=()=>{let id=b.dataset.districtApprove||b.dataset.districtDeny,x=M.districtRequests.find(y=>y.id===id);if(!x)return;x.status=b.dataset.districtApprove?'Approved by district':'Denied by district';if(x.status==='Approved by district'&&x.candidateId){let a=state.applications.find(z=>z.id===x.candidateId);if(a)a.status='Offer Pending';}if(x.status==='Approved by district'&&x.type==='Additional teacher')addTask('Post district-authorized teacher vacancy','This week','High');v94Record(`District decision: ${x.type} — ${x.status}.`);v94District();});}
function v94Moves(){v94Init();let M=state.v94;openModal('📦 Classroom Moves & Physical Preparation',`<div class="v93modal"><p>Approved assignment changes require packing, furniture, technology, curriculum materials and classroom readiness before opening day.</p><div class="v93toolbar"><button id="v94ScanMoves" class="primary">Generate moves from staffing plan</button></div>${M.roomMoves.map(x=>`<div class="compact-item"><strong>${x.name}</strong> • ${x.from} → ${x.to}<br>${x.stage} • ${x.date}<br><button class="secondary" data-move="${x.id}">Advance work order</button></div>`).join('')||'<p>No room moves recorded yet.</p>'}</div>`);$('v94ScanMoves').onclick=()=>{activeEmployees().filter(e=>e.category==='Teacher').forEach(e=>{let dest=state.v93.nextYear[e.id]?.room;if(dest&&dest!==e.room&&!M.roomMoves.some(x=>x.employeeId===e.id&&x.to===dest&&x.stage!=='Complete'))M.roomMoves.push({id:uid('move94'),employeeId:e.id,name:e.name,from:e.room,to:dest,date:state.date,stage:'Approved / packing',steps:['Approved / packing','Furniture / custodial','Technology / materials','Ready for occupancy','Complete']});});v94Moves();};document.querySelectorAll('[data-move]').forEach(b=>b.onclick=()=>{let x=M.roomMoves.find(y=>y.id===b.dataset.move);if(!x)return;x.stage=x.steps[Math.min(x.steps.length-1,x.steps.indexOf(x.stage)+1)];if(x.stage==='Complete'){let r=roomById(x.to);if(r){r.condition=clamp(r.condition+2,0,100);r.equipment=r.equipment||[];r.equipment.push('Classroom move prepared');}v94Record(`Classroom move completed: ${x.name}, ${x.to}.`);}v94Moves();});}
function v94Office(){v94Init();let M=state.v94;openModal('🖥️ Principal Office — Your Workstation',`<div class="v93modal"><p>Your office is the central management workspace. Open a document, call a meeting or walk out to supervise.</p><div class="principal-hub-grid"><button id="v94OfficeStaff">Employee Directory</button><button id="v94OfficeChart">Staffing Chart</button><button id="v94OfficeHR">HR Requests</button><button id="v94OfficeEmail">District Email</button><button id="v94OfficeCalendar">Calendar</button><button id="v94OfficeBudget">Budget</button><button id="v94OfficeDesk">Desk / Paperwork</button><button id="v94OfficeCall">Call Employee In</button><button id="v94OfficeTeam">Grade-Level Meeting</button><button id="v94OfficeWalk">Walk the Building</button></div><h3>Desk items</h3>${M.officeDesk.slice(0,5).map(x=>`<div class="compact-item">${x.text} • ${x.status}</div>`).join('')||'<p>No desk items waiting.</p>'}</div>`);let links={v94OfficeStaff:v94Careers,v94OfficeChart:v93Board,v94OfficeHR:v94District,v94OfficeEmail:v94Email,v94OfficeCalendar:v94Calendar,v94OfficeBudget:v94Strategy,v94OfficeDesk:v94Desk,v94OfficeCall:v94Call,v94OfficeTeam:v94Team,v94OfficeWalk:v83RenderMap};Object.entries(links).forEach(([id,fn])=>$(id).onclick=()=>{closeModal();fn();});}
function v94Email(){v94Init();let M=state.v94;openModal('📧 District Email',`<div class="v93modal"><button id="v94Compose" class="primary">Compose</button>${M.officeMail.map(x=>`<div class="compact-item"><strong>${x.subject}</strong> • ${x.to}<br>${x.body}</div>`).join('')||'<p>No outgoing district email yet.</p>'}</div>`);$('v94Compose').onclick=()=>{openModal('Compose District Email',`<form id="v94EmailForm" class="v93form"><label>To<input id="v94To" required value="District HR"></label><label>Subject<input id="v94Subject" required></label><label>Message<textarea id="v94Body" rows="4" required></textarea></label><button class="primary">Send (in-game)</button></form>`);$('v94EmailForm').onsubmit=ev=>{ev.preventDefault();M.officeMail.unshift({date:state.date,to:$('v94To').value,subject:$('v94Subject').value,body:$('v94Body').value});v94Email();};};}
function v94Calendar(){v94Init();openModal('📅 Principal Calendar',`<div class="v93modal"><button id="v94CalNew" class="primary">Schedule Meeting</button>${state.v94.officeCalendar.map(x=>`<div class="compact-item">${x.date} • ${x.time} — ${x.title}</div>`).join('')||'<p>No new office meetings scheduled.</p>'}</div>`);$('v94CalNew').onclick=()=>{openModal('Schedule Meeting',`<form id="v94CalForm" class="v93form"><label>Title<input id="v94CalTitle" required></label><label>Date<input id="v94CalDate" type="date" value="${state.date}" required></label><label>Time<input id="v94CalTime" type="time" value="15:30" required></label><button class="primary">Schedule</button></form>`);$('v94CalForm').onsubmit=ev=>{ev.preventDefault();state.v94.officeCalendar.push({title:$('v94CalTitle').value,date:$('v94CalDate').value,time:$('v94CalTime').value});v94Calendar();};};}
function v94Desk(){v94Init();let M=state.v94;if(!M.officeDesk.length)M.officeDesk.push(...['Review annual staffing assignments','Read pending teacher evaluations','Review purchase request','Prepare family communication'].map(text=>({id:uid('desk94'),text,status:'Pending'})));openModal('📄 Principal Desk',`<div class="v93modal">${M.officeDesk.map(x=>`<div class="compact-item">${x.text} • ${x.status}<br><button data-desk="${x.id}" class="secondary">Review / file</button></div>`).join('')}</div>`);document.querySelectorAll('[data-desk]').forEach(b=>b.onclick=()=>{let x=M.officeDesk.find(y=>y.id===b.dataset.desk);if(x)x.status='Reviewed';v94Desk();});}
function v94Call(){openModal('☎️ Call Employee to Office',`<form id="v94CallForm" class="v93form"><label>Employee<select id="v94CallEmp">${v93StaffOptions()}</select></label><label>Purpose<select id="v94CallWhy"><option>Assignment conversation</option><option>Support check-in</option><option>Evaluation conference</option><option>Career planning</option></select></label><button class="primary">Call Employee</button></form>`);$('v94CallForm').onsubmit=ev=>{ev.preventDefault();let e=state.employees.find(x=>x.id===$('v94CallEmp').value);if(!e)return;state.v94.staffLocations[e.id]='OFFICE';v94Record(`${e.name} called to principal office: ${$('v94CallWhy').value}.`);v92EmployeeConversation(e);};}
function v94Team(){openModal('👥 Convene Grade-Level Meeting',`<form id="v94TeamForm" class="v93form"><label>Grade<select id="v94TeamGrade">${GRADES.map(g=>`<option>${g}</option>`).join('')}</select></label><label>Agenda<textarea id="v94TeamAgenda" required>Review student needs, class size, and next-year staffing.</textarea></label><button class="primary">Hold Meeting</button></form>`);$('v94TeamForm').onsubmit=ev=>{ev.preventDefault();let g=$('v94TeamGrade').value;state.simMinutes+=30;v94Record(`Grade ${g} team meeting: ${$('v94TeamAgenda').value}`);closeModal();render();};}
function v94Strategy(){v94Init();let M=state.v94,S=M.strategy;openModal('🎯 Five-Year School Improvement Plan',`<div class="v93modal"><p>School: ${M.career.school} • Plan started ${S.year} • Focus: <strong>${S.focus}</strong></p><form id="v94StrategyForm" class="v93form"><label>Priority<select id="v94Focus"><option>Building and facilities</option><option>Staff retention and stability</option><option>Academic improvement</option><option>Family engagement</option></select></label><label>Measurable goal<textarea id="v94GoalText" required rows="3">Improve conditions and operational reliability through planned maintenance and regular review.</textarea></label><button class="primary">Save Strategic Goal</button></form><h3>Annual reviews</h3>${S.reviews.slice(0,5).map(x=>`<div class="compact-item">${x.date}: ${x.text}</div>`).join('')||'<p>Review progress annually.</p>'}<button id="v94Review" class="secondary">Conduct Annual Review</button></div>`);$('v94Focus').value=S.focus;$('v94StrategyForm').onsubmit=ev=>{ev.preventDefault();S.focus=$('v94Focus').value;S.goals.unshift({date:state.date,focus:S.focus,text:$('v94GoalText').value});v94Record(`School improvement goal updated: ${S.focus}.`);v94Strategy();};$('v94Review').onclick=()=>{S.reviews.unshift({date:state.date,text:`Reviewed ${S.focus}: staffing ${activeEmployees().length}, enrollment ${enrollment()}, building condition ${buildingScore()}.`});v94Strategy();};}
function v94PrincipalCareer(){v94Init();let M=state.v94;openModal('🧭 Principal Career & District Opportunities',`<div class="v93modal"><p>Current position: <strong>${M.career.role}</strong> • ${M.career.school} • ${M.career.tenure} year(s).</p><p>Other schools remain part of the district world. A career move preserves Lincoln’s history.</p>${['Principal — Jefferson Elementary','Principal — Maple Ridge Elementary','Assistant Superintendent — Operations','District Operations Director','Middle School Principal'].map(title=>`<div class="compact-item"><strong>${title}</strong><br><button class="secondary" data-job="${title}">Apply for opening</button></div>`).join('')}<h3>Applications</h3>${M.careerApplications.map(x=>`<div class="compact-item">${x.title} • ${x.status}</div>`).join('')||'<p>No applications yet.</p>'}</div>`);document.querySelectorAll('[data-job]').forEach(b=>b.onclick=()=>{let title=b.dataset.job;if(M.careerApplications.some(x=>x.title===title&&x.status==='Pending review'))return toast('Application already submitted.');M.careerApplications.unshift({id:uid('job94'),date:state.date,title,status:'Pending review',days:0});v94Record(`Principal career application submitted: ${title}.`);v94PrincipalCareer();});}
function v94PeopleMap(){v94Init();let M=state.v94,minutes=state.simMinutes;let employees=activeEmployees();employees.forEach(e=>{let room=e.room||'OFFICE';if(minutes<450)room=e.category==='Cafeteria'?'CAF':e.category==='Operations'?'ENTRY':room;else if(minutes>=690&&minutes<810&&e.category==='Cafeteria')room='CAF';else if(minutes>=900&&e.category==='Operations')room=pick(['CAF','GYM','ENTRY',e.room||'ENTRY']);else if(minutes>=930&&e.category==='Teacher')room=pick([e.room||'OFFICE','LOUNGE','OFFICE']);M.staffLocations[e.id]=room;});}
function v94Day(){v94Init();let M=state.v94;M.coaching.filter(x=>x.status==='Active').forEach(x=>x.days++);M.recruitment.forEach(x=>{let a=state.applications.find(z=>z.id===x.id);if(!a||['Hired','Withdrawn','Rejected','Offer Declined'].includes(a.status))return;x.daysWaiting++;if(x.daysWaiting>=5&&x.otherOffers>0&&Math.random()<.16){a.status='Withdrawn';x.withdrawn=true;state.inbox.unshift(msg('District HR',`${a.name} withdrew application`,'Candidate accepted another opportunity after an extended decision timeline.','medium',['Acknowledge']));}});M.careerApplications.filter(x=>x.status==='Pending review').forEach(x=>{x.days++;if(x.days>=5){x.status=pick(['Interview offered','Not selected for interview']);state.inbox.unshift(msg('District HR',`Career application update — ${x.title}`,x.status,'medium',['Acknowledge']));}});v94PeopleMap();}
function v94YearTransition(){v94Init();let M=state.v94;let plans=state.v93.nextYear;let changes=[];activeEmployees().filter(e=>e.category==='Teacher').forEach(e=>{let p=plans[e.id];if(p?.room&&p.room!==e.room)changes.push({id:uid('move94'),employeeId:e.id,name:e.name,from:e.room,to:p.room,date:state.date,stage:'Approved / packing',steps:['Approved / packing','Furniture / custodial','Technology / materials','Ready for occupancy','Complete']});});M.roomMoves.push(...changes);M.summerOperations.unshift({year:state.schoolYear,staffing:changes.length,registration:enrollment(),cleaning:'Deep cleaning scheduled',technology:'Room device moves scheduled'});M.career.tenure++;M.strategy.reviews.unshift({date:state.date,text:`Annual transition review: ${changes.length} room moves; ${enrollment()} students.`});}
function v94Hub(){v94Init();openModal('🏛️ District & Career Management — V9.4',`<div class="v93modal"><p>Staffing season, long-term careers, enrollment and district decisions now connect to the live school.</p><div class="principal-hub-grid"><button id="v94Spring">🌷 Placement Season</button><button id="v94Enrollment">📊 Enrollment & Sections</button><button id="v94Careers">👥 Employee Careers</button><button id="v94Coaching">📚 Coaching</button><button id="v94Recruiting">🤝 Hiring Competition</button><button id="v94District">🏢 Superintendent / HR</button><button id="v94Moves">📦 Classroom Moves</button><button id="v94Office">🖥️ Principal Office</button><button id="v94Strategy">🎯 Five-Year Plan</button><button id="v94Principal">🧭 Principal Career</button></div><h3>Recent decisions</h3>${state.v94.careerHistory.slice(0,5).map(x=>`<div class="compact-item">${x.date}: ${x.text}</div>`).join('')||'<p>Begin with the spring staffing season or principal office.</p>'}</div>`);let links={v94Spring:v94StaffingSeason,v94Enrollment:v94Enrollment,v94Careers:v94Careers,v94Coaching:v94Coaching,v94Recruiting:v94Recruiting,v94District:v94District,v94Moves:v94Moves,v94Office:v94Office,v94Strategy:v94Strategy,v94Principal:v94PrincipalCareer};Object.entries(links).forEach(([id,fn])=>$(id).onclick=fn);}
function v94Render(){v94Init();let host=$('v93Management')||$('v92Relationships');if(!host)return;let box=$('v94Management');if(!box){box=document.createElement('section');box.id='v94Management';box.className='card padded';host.parentNode.insertBefore(box,host.nextSibling);}let M=state.v94;box.innerHTML=`<div class="section-head"><div><h2>🏛️ District, Careers & Staffing Season</h2><p class="muted">Build your school through real staffing cycles and long-term decisions.</p></div><button class="primary" id="v94Open">Open District Management</button></div><div class="v91-strip"><button id="v94QuickOffice">🖥️ Principal Office</button><button id="v94QuickSeason">🌷 Staffing Season</button><button id="v94QuickEnroll">📊 Enrollment</button><button id="v94QuickDistrict">🏢 District <strong>${M.districtRequests.filter(x=>x.status==='Awaiting district decision').length} pending</strong></button></div>`;$('v94Open').onclick=v94Hub;$('v94QuickOffice').onclick=v94Office;$('v94QuickSeason').onclick=v94StaffingSeason;$('v94QuickEnroll').onclick=v94Enrollment;$('v94QuickDistrict').onclick=v94District;}
const v94RenderOld=render;render=function(){v94Init();v94RenderOld();v94Render();};
const v94SaveOld=save;save=function(silent=false){v94Init();try{localStorage.setItem(V94_SAVE_KEY,JSON.stringify(state));}catch(e){console.error(e);}return v94SaveOld(silent);};
const v94LoadOld=load;load=function(){let raw=localStorage.getItem(V94_SAVE_KEY);if(raw){try{state=JSON.parse(raw);v94Init();render();toast('V9.4 district management save loaded.');return;}catch(e){console.error(e);}}return v94LoadOld();};
const v94AdvanceOld=advanceDay;advanceDay=function(){v94AdvanceOld();v94Day();save(true);};
const v94YearOld=endSchoolYear;endSchoolYear=function(){v94YearTransition();v94YearOld();v94Init();};
const v94PrincipalHubOld=v91PrincipalHub;v91PrincipalHub=function(){v94PrincipalHubOld();let grid=document.querySelector('.principal-hub-grid');if(grid&&!$('v94PrincipalLink')){let b=document.createElement('button');b.id='v94PrincipalLink';b.textContent='🏛️ District & Career Management';b.onclick=()=>{closeModal();v94Hub();};grid.prepend(b);}};
const v94FinishPlanOld=v93Finalize;v93Finalize=function(){v94Init();v94FinishPlanOld();};

/* V9.4 finishing integration: map, career offers, and school-year assignment handoff. */
const v94EmployeeLocationOld=currentEmployeeLocation;
currentEmployeeLocation=function(e){if(state?.v94?.staffLocations?.[e?.id]&&state.simMinutes>=930)return state.v94.staffLocations[e.id];return v94EmployeeLocationOld(e);};
const v94TickOld=playableTick;playableTick=function(minutes=5){v94TickOld(minutes);if(state?.v94)v94PeopleMap();};
const v94YearHandoffOld=endSchoolYear;
endSchoolYear=function(){
 let planned=state?.v93?.nextYear?Object.entries(state.v93.nextYear).map(([id,p])=>({id,room:p.room,assignment:p.assignment})):[];
 v94YearHandoffOld();
 let teachers=activeEmployees().filter(e=>e.category==='Teacher'),seen=new Set();
 planned.forEach(p=>{let e=teachers.find(x=>x.id===p.id);if(!e||!roomById(p.room)?.grade||seen.has(p.room))return;seen.add(p.room);let old=e.room;if(old!==p.room){e.room=p.room;e.assignment=p.assignment||`Grade ${roomById(p.room).grade}`;e.history=e.history||[];e.history.unshift(`${state.date}: New-year classroom assignment ${old} → ${p.room}`);v94Record(`Opening-day assignment: ${e.name} moved to ${p.room}.`);}});
 state.v93.nextYear={};teachers.forEach(e=>state.v93.nextYear[e.id]={room:e.room,assignment:e.assignment,fte:e.fte});
 state.v94.placement=[];state.v94.projections[state.schoolYear]=Object.fromEntries(GRADES.map(g=>[g,state.students.filter(x=>x.status==='Active'&&String(x.grade)===String(g)).length]));
};
const v94CareerPanelOld=v94PrincipalCareer;
v94PrincipalCareer=function(){v94CareerPanelOld();let accepted=state.v94.careerApplications.filter(x=>x.status==='Interview offered');if(!accepted.length)return;let modal=$('modalBody');if(!modal)return;let box=document.createElement('div');box.className='compact-item';box.innerHTML='<h3>Interview invitations</h3>'+accepted.map(x=>`<button class="secondary" data-career-interview="${x.id}">Interview for ${x.title}</button>`).join('');modal.appendChild(box);box.querySelectorAll('[data-career-interview]').forEach(b=>b.onclick=()=>{let x=state.v94.careerApplications.find(y=>y.id===b.dataset.careerInterview);if(!x)return;x.status='Interview completed';x.days=0;v94Record(`Interviewed for ${x.title}.`);v94PrincipalCareer();});};
const v94CareerDayOld=v94Day;
v94Day=function(){v94CareerDayOld();state.v94.careerApplications.filter(x=>x.status==='Interview completed').forEach(x=>{x.days++;if(x.days>=4){x.status=Math.random()<.45?'Offer received':'Not selected';state.inbox.unshift(msg('District HR',`Career application: ${x.title}`,x.status,'medium',['Acknowledge']));}});};



/* V9.5: playable PC school operations. All actions update the persistent school state. */
const V95_SAVE_KEY='lincolnElementaryRealPrincipalV95';
function v95Init(){v94Init();let V=state.v95=state.v95||{};for(let k of ['coverage','districtDecisions','projects','staffHistory','summerTasks','budgetLedger','scheduleChanges','personnelActions'])if(!Array.isArray(V[k]))V[k]=[];V.schedule=V.schedule||{};V.duties=V.duties||{};V.office=V.office||{tab:'Desk'};V.district=V.district||{role:'Principal',schools:[{name:'Lincoln Elementary',principal:'You',enrollment:enrollment()},{name:'Jefferson Elementary',principal:'Principal Carter',enrollment:435},{name:'Maple Ridge Elementary',principal:'Principal Lopez',enrollment:390}]};V.construction=V.construction||{rooms:[]};V.staffFiles=V.staffFiles||{};activeEmployees().forEach(e=>{V.staffFiles[e.id]=V.staffFiles[e.id]||{meetings:[],training:[],evaluations:[],assignmentHistory:[],attendance:[],notes:[]};});}
function v95Log(text){v95Init();state.schoolHistory.unshift({date:state.date,text});}
function v95Go(fn){closeModal();fn();}
function v95Location(e){let override=state.v95?.duties?.[e.id];if(override&&override.date===state.date&&state.simMinutes>=override.start&&state.simMinutes<override.end)return override.room;return state.v94?.staffLocations?.[e.id]||e.room||'OFFICE';}
function v95Campus(){v95Init();let rooms=state.rooms.filter(r=>r.grade||['CAF','GYM','ART','MUSIC','LIB','OFFICE','ENTRY','LOUNGE'].includes(r.id));let here=state.principalLocation||'OFFICE';openModal('🏫 Live Campus — '+mins12(state.simMinutes),`<div class="v95campus"><p>Click a room to walk there. Staff shown are based on current assignments and duty overrides. ${state.v95.district.role==='Superintendent'?'District view active.':'Principal view active.'}</p><div class="v95roomgrid">${rooms.map(r=>{let staff=activeEmployees().filter(e=>v95Location(e)===r.id);return `<button class="v95room ${here===r.id?'v95here':''}" data-room95="${r.id}"><strong>${r.id}</strong><small>${r.name||r.grade||r.type}</small><span>${staff.map(e=>e.name.split(' ')[0]).join(', ')||'—'}</span></button>`}).join('')}</div><div class="v93toolbar"><button id="v95MapSchedule" class="secondary">Master Schedule</button><button id="v95MapCoverage" class="secondary">Coverage Board</button></div></div>`);document.querySelectorAll('[data-room95]').forEach(b=>b.onclick=()=>v95Room(b.dataset.room95));$('v95MapSchedule').onclick=v95Schedule;$('v95MapCoverage').onclick=v95Coverage;}
function v95Room(id){let r=roomById(id);if(!r)return;state.principalLocation=id;let staff=activeEmployees().filter(e=>v95Location(e)===id);openModal(`🚪 ${r.name||id} — ${mins12(state.simMinutes)}`,`<p>Current room: ${id}. ${r.grade?'Grade '+r.grade+' • '+studentsInRoom(id).length+' students.':''}</p>${staff.map(e=>`<div class="compact-item"><strong>${e.name}</strong> • ${e.position}<br><button data-person95="${e.id}" class="primary">Open personnel file / interact</button></div>`).join('')||'<p>No staff scheduled here at this moment.</p>'}<div class="v93toolbar"><button id="v95RoomBack" class="secondary">Back to campus</button>${r.grade?'<button id="v95RoomObs" class="secondary">Walkthrough</button>':''}</div>`);document.querySelectorAll('[data-person95]').forEach(b=>b.onclick=()=>v95Personnel(b.dataset.person95));$('v95RoomBack').onclick=v95Campus;if($('v95RoomObs'))$('v95RoomObs').onclick=()=>{let e=activeEmployees().find(e=>e.room===id&&e.category==='Teacher');if(e)observeTeacher(e,false);else toast('No assigned teacher available to observe.');};}
function v95Personnel(id){v95Init();let e=state.employees.find(x=>x.id===id);if(!e)return;let f=state.v95.staffFiles[id],c=state.v94.staffCareers[id]||{};openModal(`👤 Personnel File — ${e.name}`,`<div class="v93modal"><h3>${e.name}</h3><p>${e.position} • ${e.category} • ${e.room||'Unassigned'} • ${e.status||'Active'}</p><p>Experience: ${e.experience||0} years • Degree: ${e.degree||'Not recorded'} • FTE: ${e.fte||1} • License: ${e.license||'Verify'}</p><p>Career goal: ${c.goal||'Not recorded'} • Preferred grades: ${(c.preferredGrades||[]).join(', ')||'Not recorded'}</p><div class="v93toolbar"><button id="v95Meet" class="primary">Meet</button><button id="v95Observe" class="secondary">Observe</button><button id="v95Duty" class="secondary">Assign duty</button><button id="v95Train" class="secondary">Training / mentor</button><button id="v95Transfer" class="secondary">Transfer / assignment</button></div><h3>Personnel history</h3>${[...f.meetings,...f.training,...f.evaluations,...f.assignmentHistory,...f.attendance].slice(-12).reverse().map(x=>`<div class="compact-item">${x.date}: ${x.text}</div>`).join('')||'<p>No entries yet.</p>'}</div>`);$('v95Meet').onclick=()=>{f.meetings.push({date:state.date,text:'Principal meeting / check-in'});v92EmployeeConversation(e);};$('v95Observe').onclick=()=>{if(e.category==='Teacher')observeTeacher(e,false);else toast('Walkthroughs are available for classroom teachers.');};$('v95Duty').onclick=()=>v95AssignDuty(id);$('v95Train').onclick=()=>{f.training.push({date:state.date,text:'Professional development and mentor check-in scheduled'});addTask(`PD/mentor follow-up: ${e.name}`,'This week','Normal');v95Personnel(id);};$('v95Transfer').onclick=v93Board;}
function v95Schedule(){v95Init();let rooms=state.rooms.filter(r=>r.grade),V=state.v95;openModal('🗓️ Master Schedule & Conflict Checker',`<div class="v93modal"><p>Assign a staff member to a room and time block. Conflicting assignments are blocked.</p><form id="v95ScheduleForm" class="v93form"><label>Employee<select id="v95ScheduleEmp">${v93StaffOptions()}</select></label><label>Location<select id="v95ScheduleRoom">${state.rooms.map(r=>`<option value="${r.id}">${r.id} — ${r.name||r.type}</option>`).join('')}</select></label><label>Start minute after midnight<input id="v95Start" type="number" min="360" max="1080" value="600" required></label><label>End minute<input id="v95End" type="number" min="361" max="1140" value="645" required></label><label>Activity<select id="v95Activity"><option>Instruction</option><option>Specials</option><option>Lunch</option><option>Recess duty</option><option>Intervention</option><option>Coverage</option><option>Custodial work</option><option>Cafeteria service</option></select></label><button class="primary">Assign time block</button></form><h3>Today's scheduled assignments</h3>${Object.entries(V.schedule).flatMap(([id,a])=>a.map(x=>`<div class="compact-item">${state.employees.find(e=>e.id===id)?.name||id}: ${x.start}–${x.end} • ${x.room} • ${x.activity}</div>`)).slice(0,35).join('')||'<p>No custom time blocks. Default staff assignments apply.</p>'}</div>`);$('v95ScheduleForm').onsubmit=ev=>{ev.preventDefault();let id=$('v95ScheduleEmp').value,room=$('v95ScheduleRoom').value,start=+$('v95Start').value,end=+$('v95End').value,activity=$('v95Activity').value;if(end<=start)return toast('End time must be after start.');let blocks=V.schedule[id]||[];if(blocks.some(x=>start<x.end&&end>x.start))return toast('Conflict: employee already assigned during that time.');V.schedule[id]=blocks;blocks.push({date:state.date,start,end,room,activity});V.scheduleChanges.unshift({date:state.date,id,room,start,end,activity});v95Log(`Master schedule: ${state.employees.find(e=>e.id===id)?.name} assigned to ${room} (${activity}).`);v95Schedule();};}
function v95AssignDuty(id){let e=state.employees.find(x=>x.id===id);if(!e)return;openModal(`📍 Assign ${e.name}`,`<form id="v95DutyForm" class="v93form"><label>Room<select id="v95DutyRoom">${state.rooms.map(r=>`<option value="${r.id}">${r.id} — ${r.name||r.type}</option>`).join('')}</select></label><label>Minutes on duty<input id="v95DutyMins" type="number" min="5" max="240" value="45"></label><button class="primary">Assign now</button></form>`);$('v95DutyForm').onsubmit=ev=>{ev.preventDefault();let room=$('v95DutyRoom').value,duration=+$('v95DutyMins').value;state.v95.duties[id]={date:state.date,room,start:state.simMinutes,end:state.simMinutes+duration};state.v95.staffFiles[id].assignmentHistory.push({date:state.date,text:`Temporary duty: ${room} for ${duration} minutes`});v95Log(`${e.name} reassigned to ${room} for ${duration} minutes.`);v95Campus();};}
function v95Coverage(){v95Init();let V=state.v95;let absent=(state.absences||[]).map(a=>({id:a.employeeId||a.id||a.staffId,name:a.name||a.employee||'Absent staff',room:a.room||''}));let uncovered=(state.coverage||[]).filter(c=>c.status==='Uncovered');openModal('📞 Attendance & Coverage Board',`<div class="v93modal"><h3>Today's call-offs: ${absent.length} • Uncovered assignments: ${uncovered.length}</h3>${absent.map(a=>`<div class="compact-item">${a.name} • ${a.room||'Assignment pending'}</div>`).join('')||'<p>No recorded absences.</p>'}${uncovered.map(c=>`<div class="compact-item">Uncovered: ${c.room||c.assignment||c.teacher||'Position'}</div>`).join('')}<form id="v95CoverageForm" class="v93form"><label>Assignment needing coverage<input id="v95CoverRoom" required placeholder="e.g. 108 or Cafeteria"></label><label>Coverage employee<select id="v95CoverEmp">${v93StaffOptions()}</select></label><label>Minutes<input id="v95CoverMins" type="number" min="15" max="480" value="90"></label><button class="primary">Assign coverage</button></form><h3>Coverage decisions</h3>${V.coverage.filter(x=>x.date===state.date).map(x=>`<div class="compact-item">${x.name} → ${x.room}, ${x.minutes} min • ${x.status}</div>`).join('')||'<p>None yet.</p>'}</div>`);$('v95CoverageForm').onsubmit=ev=>{ev.preventDefault();let id=$('v95CoverEmp').value,e=state.employees.find(x=>x.id===id),room=$('v95CoverRoom').value.trim(),minutes=+$('v95CoverMins').value;if(!e)return;let r=roomById(room);if(r)state.v95.duties[id]={date:state.date,room,start:state.simMinutes,end:state.simMinutes+minutes};V.coverage.unshift({date:state.date,employeeId:id,name:e.name,room,minutes,status:'Assigned'});state.finance.spentOps+=(e.category==='Teacher'?90:45);v95Log(`${e.name} covering ${room} for ${minutes} minutes.`);v95Coverage();};}
function v95Budget(){v95Init();let F=state.finance,V=state.v95;let ops=typeof availableOperating==='function'?availableOperating():0;openModal('💰 School Financial Control',`<div class="v93modal"><h3>Operating funds available: ${money(ops)}</h3><p>Payroll projection: ${money(payroll())} • Building condition: ${buildingScore()}</p><p>Current operating spend: ${money(F.spentOps||0)} • Recent coverage cost decisions: ${V.coverage.length}</p><form id="v95BudgetForm" class="v93form"><label>Expense / request<input id="v95Expense" required placeholder="Classroom supplies, overtime, equipment"></label><label>Amount<input id="v95Amount" type="number" min="1" value="250" required></label><label>Type<select id="v95ExpenseType"><option>Operating purchase</option><option>District approval request</option><option>Professional development</option><option>Maintenance</option></select></label><button class="primary">Record decision</button></form><h3>Ledger</h3>${V.budgetLedger.slice(0,15).map(x=>`<div class="compact-item">${x.date}: ${x.text} • ${money(x.amount)} • ${x.status}</div>`).join('')||'<p>No V9.5 transactions yet.</p>'}</div>`);$('v95BudgetForm').onsubmit=ev=>{ev.preventDefault();let amount=+$('v95Amount').value,text=$('v95Expense').value.trim(),type=$('v95ExpenseType').value;if(type!=='District approval request'&&amount>availableOperating())return toast('Insufficient operating funds. Submit a district request.');let entry={date:state.date,text,amount,type,status:type==='District approval request'?'Pending district':'Recorded'};V.budgetLedger.unshift(entry);if(entry.status==='Recorded')F.spentOps+=amount;else state.v94.districtRequests.unshift({id:uid('district95'),type:'Budget',reason:`${text} — ${money(amount)}`,status:'Awaiting district decision',date:state.date,amount});v95Log(`${type}: ${text} — ${money(amount)}.`);v95Budget();};}
function v95Board(){v95Init();let requests=state.v94.districtRequests.filter(x=>x.status==='Awaiting district decision');openModal('🏛️ Independent District Review',`<div class="v93modal"><p>District decisions are evaluated using the school’s enrollment, available operating funds, and request type. You can submit requests but cannot select the outcome.</p><button id="v95ReviewRequests" class="primary">Advance district review (one meeting)</button><h3>Pending requests: ${requests.length}</h3>${requests.map(x=>`<div class="compact-item">${x.type} • ${x.reason||''} • ${x.status}</div>`).join('')||'<p>No pending requests.</p>'}<h3>Decision history</h3>${state.v95.districtDecisions.slice(0,15).map(x=>`<div class="compact-item">${x.date}: ${x.type} — ${x.status} (${x.reason})</div>`).join('')||'<p>No decisions yet.</p>'}</div>`);$('v95ReviewRequests').onclick=()=>{let count=0;requests.forEach(x=>{let amount=x.amount||0,capacity=availableOperating(),roll=enrollment(),chance=amount>capacity?0.27:roll>450?0.68:0.52;let rollResult=Math.random();x.status=rollResult<chance?'Approved':rollResult<chance+.23?'More information requested':'Deferred';x.reviewDate=state.date;state.v95.districtDecisions.unshift({date:state.date,type:x.type,status:x.status,reason:x.reason||''});if(x.status==='Approved'&&x.amount){state.finance.spentOps+=x.amount;let ledger=state.v95.budgetLedger.find(z=>z.status==='Pending district'&&x.reason?.includes(z.text));if(ledger)ledger.status='Approved / recorded';}count++;});if(!count)toast('No requests awaiting review.');else v95Log(`District meeting reviewed ${count} request(s).`);v95Board();};}
function v95Construction(){v95Init();let P=state.v95.projects;openModal('🏗️ Construction & Campus Expansion',`<div class="v93modal"><p>Building projects require district review before construction. Approved additions progress across school days and appear as usable rooms when complete.</p><form id="v95BuildForm" class="v93form"><label>Project<select id="v95BuildType"><option>Classroom addition</option><option>Library renovation</option><option>Playground upgrade</option><option>HVAC replacement</option></select></label><label>Estimated cost<input id="v95BuildCost" type="number" min="1000" value="350000"></label><button class="primary">Submit project</button></form>${P.map(x=>`<div class="compact-item"><strong>${x.name}</strong> • ${money(x.cost)}<br>${x.stage} • ${x.days} days in process</div>`).join('')||'<p>No projects yet.</p>'}</div>`);$('v95BuildForm').onsubmit=ev=>{ev.preventDefault();let x={id:uid('build95'),name:$('v95BuildType').value,cost:+$('v95BuildCost').value,stage:'District review',days:0,approved:false};P.unshift(x);state.v94.districtRequests.unshift({id:uid('district95'),type:'Capital project',reason:`${x.name} — ${money(x.cost)}`,projectId:x.id,status:'Awaiting district decision',date:state.date});v95Construction();};}
function v95Summer(){v95Init();let V=state.v95;if(!V.summerTasks.length)V.summerTasks=['Finalize classroom assignments','Resolve vacancies','Inspect classroom furniture','Complete floor and deep cleaning','Move technology and materials','Review registrations','Confirm specials and lunch schedules','Complete opening-day readiness'].map(text=>({text,done:false}));openModal('☀️ Summer & Opening-Day Readiness',`<div class="v93modal"><p>Finish tasks before starting the next school year. Unfinished work will be logged as opening-day problems.</p>${V.summerTasks.map((x,i)=>`<div class="compact-item"><label><input type="checkbox" data-summer95="${i}" ${x.done?'checked':''}> ${x.text}</label></div>`).join('')}<button id="v95StartYear" class="primary">Start next school year</button></div>`);document.querySelectorAll('[data-summer95]').forEach(b=>b.onchange=()=>{V.summerTasks[+b.dataset.summer95].done=b.checked;});$('v95StartYear').onclick=()=>{let undone=V.summerTasks.filter(x=>!x.done);if(undone.length&&!confirm(`${undone.length} tasks remain. Start the school year anyway?`))return;endSchoolYear();undone.forEach(x=>{addTask(`Opening-day issue: ${x.text}`,'Today','High');});V.summerTasks=[];v95Log(`Opening day started with ${undone.length} outstanding preparation tasks.`);closeModal();render();};}
function v95Superintendent(){v95Init();let D=state.v95.district,role=state.v94.career.role||'';if(/superintendent/i.test(role))D.role='Superintendent';openModal('🏢 District Leadership Career',`<div class="v93modal"><p>Current gameplay role: <strong>${D.role}</strong>. Career offers must be received through the existing application system before you can accept a district role.</p>${state.v94.careerApplications.filter(x=>x.status==='Offer received').map(x=>`<div class="compact-item">${x.title} — offer received <button class="primary" data-accept95="${x.id}">Accept position</button></div>`).join('')||'<p>No outstanding offers. Apply through Principal Career and advance days to receive decisions.</p>'}<h3>District schools</h3>${D.schools.map(x=>`<div class="compact-item">${x.name} • ${x.principal} • ${x.enrollment} students</div>`).join('')}<button id="v95DistrictBoard" class="secondary">District decision queue</button>${D.role==='Superintendent'?'<button id="v95DistrictBudget" class="secondary">District financial requests</button>':''}</div>`);document.querySelectorAll('[data-accept95]').forEach(b=>b.onclick=()=>{let x=state.v94.careerApplications.find(y=>y.id===b.dataset.accept95);if(!x)return;x.status='Accepted';state.v94.career.role=x.title;D.role=/superintendent/i.test(x.title)?'Superintendent':'Principal';state.v94.career.school=x.title.split('—')[1]?.trim()||state.v94.career.school;v95Log(`Accepted career offer: ${x.title}. Lincoln's previous records retained.`);v95Superintendent();});$('v95DistrictBoard').onclick=v95Board;if($('v95DistrictBudget'))$('v95DistrictBudget').onclick=v95Budget;}
function v95Office(){v95Init();openModal('🖥️ Principal Office — Desktop',`<div class="v93modal"><p>Use your desk to manage the school proactively. Staff changes, finances, and coverage persist in your save.</p><div class="principal-hub-grid"><button id="v95OfficeCampus">Campus</button><button id="v95OfficeStaff">Personnel files</button><button id="v95OfficeSchedule">Master schedule</button><button id="v95OfficeCoverage">Call-offs & coverage</button><button id="v95OfficeBudget">Finance</button><button id="v95OfficeBoard">District / board</button><button id="v95OfficeBuild">Construction</button><button id="v95OfficeSummer">Summer</button><button id="v95OfficeCareer">Career / superintendent</button><button id="v95OfficeMail">Email</button><button id="v95OfficeCalendar">Calendar</button><button id="v95OfficeDesk">Paperwork</button></div></div>`);let links={v95OfficeCampus:v95Campus,v95OfficeStaff:v95StaffDirectory,v95OfficeSchedule:v95Schedule,v95OfficeCoverage:v95Coverage,v95OfficeBudget:v95Budget,v95OfficeBoard:v95Board,v95OfficeBuild:v95Construction,v95OfficeSummer:v95Summer,v95OfficeCareer:v95Superintendent,v95OfficeMail:v94Mail,v95OfficeCalendar:v94Calendar,v95OfficeDesk:v94Desk};Object.entries(links).forEach(([id,fn])=>{if($(id))$(id).onclick=fn;});}
function v95StaffDirectory(){openModal('👥 Employee Directory',`<div class="v93modal">${activeEmployees().map(e=>`<div class="compact-item"><strong>${e.name}</strong> • ${e.position} • ${e.room||'Unassigned'} <button class="secondary" data-staff95="${e.id}">Personnel file</button></div>`).join('')}</div>`);document.querySelectorAll('[data-staff95]').forEach(b=>b.onclick=()=>v95Personnel(b.dataset.staff95));}
function v95Hub(){v95Init();openModal('🏫 Real Principal Experience — V9.5',`<div class="v93modal"><p>A connected PC management workspace: walk the building, assign staff, resolve coverage, control spending, and prepare for next year.</p><div class="principal-hub-grid"><button id="v95HCampus">🏫 Living campus</button><button id="v95HStaff">👥 Personnel files</button><button id="v95HSchedule">🗓️ Master schedule</button><button id="v95HCoverage">📞 Attendance & subs</button><button id="v95HOffice">🖥️ Principal office</button><button id="v95HBudget">💰 Budget</button><button id="v95HBoard">🏛️ District board</button><button id="v95HBuild">🏗️ Construction</button><button id="v95HSummer">☀️ Summer</button><button id="v95HCareer">🧭 Career</button></div></div>`);Object.entries({v95HCampus:v95Campus,v95HStaff:v95StaffDirectory,v95HSchedule:v95Schedule,v95HCoverage:v95Coverage,v95HOffice:v95Office,v95HBudget:v95Budget,v95HBoard:v95Board,v95HBuild:v95Construction,v95HSummer:v95Summer,v95HCareer:v95Superintendent}).forEach(([id,fn])=>$(id).onclick=fn);}
function v95Render(){v95Init();let host=$('v94Management');if(!host)return;let box=$('v95Management');if(!box){box=document.createElement('section');box.id='v95Management';box.className='card padded';host.parentNode.insertBefore(box,host.nextSibling);}box.innerHTML=`<div class="section-head"><div><h2>🏫 Real Principal Experience</h2><p class="muted">Manage people, schedules, coverage, finances, facilities and the district.</p></div><button class="primary" id="v95Open">Open V9.5 Management</button></div><div class="v91-strip"><button id="v95QuickCampus">🏫 Campus</button><button id="v95QuickSchedule">🗓️ Schedule</button><button id="v95QuickStaff">👥 Personnel</button><button id="v95QuickOffice">🖥️ Office</button></div>`;$('v95Open').onclick=v95Hub;$('v95QuickCampus').onclick=v95Campus;$('v95QuickSchedule').onclick=v95Schedule;$('v95QuickStaff').onclick=v95StaffDirectory;$('v95QuickOffice').onclick=v95Office;}
function v95Day(){v95Init();let V=state.v95;V.projects.forEach(p=>{p.days++;let request=state.v94.districtRequests.find(x=>x.projectId===p.id);if(p.stage==='District review'&&request?.status==='Approved'){p.stage='Planning / permits';p.days=0;}else if(p.stage==='Planning / permits'&&p.days>=3){p.stage='Construction';p.days=0;}else if(p.stage==='Construction'&&p.days>=12){p.stage='Inspection';p.days=0;}else if(p.stage==='Inspection'&&p.days>=3){p.stage='Complete';if(p.name==='Classroom addition'){let id='NEW'+(V.construction.rooms.length+1);let r={id,name:'New Classroom '+id,type:'classroom',grade:null,condition:100,cleanliness:100,capacity:24};state.rooms.push(r);V.construction.rooms.push(id);}v95Log(`Construction complete: ${p.name}.`);}});V.duties={};}
const v95RenderOld=render;render=function(){v95Init();v95RenderOld();v95Render();};
const v95SaveOld=save;save=function(silent=false){v95Init();try{localStorage.setItem(V95_SAVE_KEY,JSON.stringify(state));}catch(e){console.error(e);}return v95SaveOld(silent);};
const v95LoadOld=load;load=function(){let raw=localStorage.getItem(V95_SAVE_KEY);if(raw){try{state=JSON.parse(raw);v95Init();render();toast('V9.5 save loaded.');return;}catch(e){console.error(e);}}return v95LoadOld();};
const v95AdvanceOld=advanceDay;advanceDay=function(){v95AdvanceOld();v95Day();save(true);};
const v95PrincipalOld=v91PrincipalHub;v91PrincipalHub=function(){v95PrincipalOld();let grid=document.querySelector('.principal-hub-grid');if(grid&&!$('v95PrincipalLink')){let b=document.createElement('button');b.id='v95PrincipalLink';b.textContent='🏫 Real Principal Experience';b.onclick=v95Hub;grid.prepend(b);}};
const v95CurrentLocationOld=currentEmployeeLocation;currentEmployeeLocation=function(e){let duty=state?.v95?.duties?.[e?.id];if(duty&&duty.date===state.date&&state.simMinutes>=duty.start&&state.simMinutes<duty.end)return duty.room;return v95CurrentLocationOld(e);};
const v95BoardOld=v95Board;v95Board=function(){v95BoardOld();let original=$('v95ReviewRequests');if(!original)return;original.onclick=()=>{let requests=state.v94.districtRequests.filter(x=>x.status==='Awaiting district decision');requests.forEach(x=>{let chance=x.amount>availableOperating()?.28:.58;let roll=Math.random();x.status=roll<chance?'Approved':roll<chance+.23?'More information requested':'Deferred';x.reviewDate=state.date;state.v95.districtDecisions.unshift({date:state.date,type:x.type,status:x.status,reason:x.reason||''});if(x.status==='Approved'&&x.amount){state.finance.spentOps+=x.amount;let ledger=state.v95.budgetLedger.find(z=>z.status==='Pending district'&&x.reason?.includes(z.text));if(ledger)ledger.status='Approved / recorded';}});v95Log(`District reviewed ${requests.length} requests.`);v95Board();};};



/* V9.6: Autonomous departments and connected school operations */
const V96_SAVE_KEY='lincolnElementaryAutonomousSchoolV96';
function v96Init(){v95Init();let V=state.v96=state.v96||{};
 for(let k of ['activity','decisions','requests','staffNotices','enrollmentChanges','familyContacts','transportIssues','custodialRoutes','cafeteriaDays','development','classroomMoves','annualTasks'])V[k]=Array.isArray(V[k])?V[k]:[];
 V.delegation=V.delegation||{Office:'Routine coverage',Instruction:'Classroom matters',Custodial:'Routine operations',Cafeteria:'Routine operations',Transportation:'Routine routing',Counseling:'Routine student support'};
 V.morale=V.morale||{};V.transport=V.transport||{};V.kitchen=V.kitchen||{stock:650,capacity:900,oven:'Working',waste:0};V.lastDaily=V.lastDaily||'';V.lastTick=V.lastTick||-1;V.phase=V.phase||'Fall';
 activeEmployees().forEach(e=>{V.morale[e.id]=V.morale[e.id]||{workload:0,stability:0,support:0,notes:[]};});
 state.students.filter(x=>x.status==='Active').forEach(st=>{if(!V.transport[st.id])V.transport[st.id]={mode:st.transport||'Bus',route:st.transport==='Bus'?'Bus '+(10+Math.abs(String(st.id).split('').reduce((a,c)=>a+c.charCodeAt(0),0))%8):'None'};});
}
function v96Record(text,area='Operations',requires=false,options=[]){v96Init();let x={id:uid('aut96'),date:state.date,time:mins12(state.simMinutes),area,text,status:requires?'Needs principal':'Handled by staff',options};V96().activity.unshift(x);V96().activity=V96().activity.slice(0,120);if(requires)V96().decisions.unshift(x);return x;}
function V96(){return state.v96;}
function v96Autonomy(){v96Init();let V=V96(),area=pick(['Office','Instruction','Custodial','Cafeteria','Transportation','Counseling']);let reports={Office:['Secretary verified daily attendance and followed up on a missing note.','Secretary arranged routine classroom coverage.','Office completed registration paperwork.'],Instruction:['Grade-level team adjusted a lesson after reviewing student work.','Teacher contacted a family about routine classroom progress.','Specials teacher coordinated a schedule change with a colleague.'],Custodial:['Head custodian redistributed rooms to balance tonight’s cleaning.','Custodian completed a routine work order.','Custodial team reset a room after a school activity.'],Cafeteria:['Kitchen lead adjusted the lunch serving line.','Cooks counted inventory and prepared the next meal.','Cafeteria manager resolved a routine lunch coverage gap.'],Transportation:['Transportation updated a student stop assignment.','Bus dispatcher resolved a minor route delay.','Office confirmed a pickup change with transportation.'],Counseling:['Counselor completed a student check-in.','Counselor facilitated a peer conflict conversation.','Counselor contacted a family about routine student support.']};let text=pick(reports[area]);let delegated=V.delegation[area]!=='Principal approval';v96Record(text,area,!delegated,delegated?[]:['Approve staff action','Delegate routine actions']);}
function v96Tick(){v96Init();let V=V96(),slot=Math.floor(state.simMinutes/25);if(V.lastTick===slot)return;V.lastTick=slot;if(Math.random()<.38)v96Autonomy();if(Math.random()<.09)v96StaffRequest();}
function v96StaffRequest(){let e=pick(activeEmployees());if(!e)return;let role=e.category;let text=role==='Teacher'?pick(['Requests a different grade next year','Requests instructional materials','Requests a mentoring opportunity']):role==='Cafeteria'?'Requests coverage for the lunch rush':role==='Operations'?'Requests a repair or equipment replacement':'Requests a schedule or duty adjustment';let x={id:uid('notice96'),employeeId:e.id,name:e.name,request:text,date:state.date,status:'Awaiting review'};V96().staffNotices.unshift(x);v96Record(`${e.name}: ${text}`,'Staffing',true,['Meet with employee','Approve request','Defer']);}
function v96ReviewDecision(id){let x=V96().decisions.find(y=>y.id===id);if(!x)return;openModal('Principal decision',`<h3>${x.area}</h3><p>${x.text}</p><div class="actions"><button id="v96Approve" class="primary">Approve / address</button><button id="v96Delegate" class="secondary">Delegate to department</button><button id="v96Later" class="secondary">Review later</button></div>`);$('v96Approve').onclick=()=>{x.status='Principal addressed';V96().decisions=V96().decisions.filter(y=>y.id!==id);if(x.area==='Staffing')addTask(`Follow up: ${x.text}`,'This week','Normal');v96Record(`Principal addressed: ${x.text}`,x.area);v96Dashboard();};$('v96Delegate').onclick=()=>{x.status='Delegated';V96().decisions=V96().decisions.filter(y=>y.id!==id);v96Record(`Principal delegated: ${x.text}`,x.area);v96Dashboard();};$('v96Later').onclick=v96Dashboard;}
function v96EnrollmentDay(){v96Init();let V=V96();if(Math.random()>.23)return;let active=state.students.filter(x=>x.status==='Active');if(!active.length)return;let leaving=Math.random()<.32,st=pick(active);if(leaving){st.status='Transferred Out';st.history=st.history||[];st.history.unshift(`${state.date}: Transferred out.`);delete V.transport[st.id];V.enrollmentChanges.unshift({date:state.date,student:st.first+' '+st.last,kind:'Transferred out',grade:st.grade});v96Record(`${st.first} ${st.last} transferred out; roster and transportation updated.`,'Enrollment');return;}
 let fam=pick(state.families),grade=pick(GRADES),rooms=state.rooms.filter(r=>String(r.grade)===String(grade));if(!rooms.length)return;rooms.sort((a,b)=>state.students.filter(x=>x.status==='Active'&&x.room===a.id).length-state.students.filter(x=>x.status==='Active'&&x.room===b.id).length);let room=rooms[0],count=state.students.filter(x=>x.status==='Active'&&x.room===room.id).length;let kid={id:uid('stu'),first:pick(firstNames),last:fam?.lastName||pick(lastNames),grade,room:room.id,familyId:fam?.id,transport:'Bus',status:'Active',attendance:100,reading:rnd(40,80),math:rnd(40,80),history:[`${state.date}: Registered and placed in ${room.id}.`]};state.students.push(kid);V.transport[kid.id]={mode:'Bus',route:'Bus '+(10+rnd(0,7))};V.enrollmentChanges.unshift({date:state.date,student:kid.first+' '+kid.last,kind:'Registered',grade,room:room.id});v96Record(`New registration: ${kid.first} ${kid.last}, Grade ${grade}, Room ${room.id}. Transportation assigned.`,'Enrollment',count>=23,['Review class sizes','Request staffing']);}
function v96Transport(){v96Init();let V=V96(),routes={};state.students.filter(x=>x.status==='Active').forEach(s=>{let t=V.transport[s.id];if(t?.mode==='Bus')routes[t.route]=(routes[t.route]||0)+1;});let overload=Object.entries(routes).filter(([,n])=>n>52);openModal('🚌 Student Transportation',`<div class="v93modal"><p>Student-level transportation assignments; route planning reflects registrations and transfers.</p>${Object.entries(routes).sort().map(([r,n])=>`<div class="compact-item">${r}: <strong>${n} riders</strong> ${n>52?'⚠️ Over capacity':''}</div>`).join('')}<p>${overload.length} route(s) require capacity review.</p><button class="primary" id="v96RouteFix">Ask transportation to rebalance routes</button></div>`);$('v96RouteFix').onclick=()=>{let buses=Object.keys(routes);let busStudents=state.students.filter(x=>x.status==='Active'&&V.transport[x.id]?.mode==='Bus');busStudents.forEach((s,i)=>V.transport[s.id].route=buses[i%buses.length]||'Bus 10');v96Record('Transportation redistributed bus riders across existing routes.','Transportation');v96Transport();};}
function v96Custodial(){v96Init();let V=V96(),staff=activeEmployees().filter(e=>e.category==='Operations'||/custod/i.test(e.position||'')),rooms=state.rooms.filter(r=>r.grade||['GYM','CAF','LIB','OFFICE'].includes(r.id));if(!staff.length){openModal('🧹 Custodial Operations','<p>No custodial staff are currently available. Review HR and coverage.</p>');return;}if(!V.custodialRoutes.length)V.custodialRoutes=rooms.map((r,i)=>({room:r.id,employeeId:staff[i%staff.length].id,status:'Assigned'}));openModal('🧹 Custodial Route Management',`<div class="v93modal"><p>Assignments persist; call-offs and special events can create uncovered rooms.</p>${staff.map(e=>`<div class="compact-item"><strong>${e.name}</strong> • ${V.custodialRoutes.filter(r=>r.employeeId===e.id).length} areas <button class="secondary" data-cust96="${e.id}">Rebalance routes</button></div>`).join('')}<button id="v96Inspect" class="primary">Inspect completed cleaning</button></div>`);document.querySelectorAll('[data-cust96]').forEach(b=>b.onclick=()=>{let target=b.dataset.cust96,other=staff.find(e=>e.id!==target);if(!other)return toast('Another employee is needed to rebalance.');let rr=V.custodialRoutes.find(r=>r.employeeId===target);if(rr){rr.employeeId=other.id;v96Record(`Reassigned ${rr.room} cleaning to ${other.name}.`,'Custodial');v96Custodial();}});$('v96Inspect').onclick=()=>{let r=pick(V.custodialRoutes);if(r){r.status='Inspected';let room=roomById(r.room);if(room)room.cleanliness=clamp((room.cleanliness||85)+2,0,100);v96Record(`Principal inspected custodial cleaning in ${r.room}.`,'Custodial');}v96Custodial();};}
function v96Kitchen(){v96Init();let K=V96().kitchen;openModal('🍽️ Cafeteria Management',`<div class="v93modal"><h3>Kitchen operations</h3><p>Food stock: ${K.stock} meals • Daily capacity: ${K.capacity} • Oven: ${K.oven} • Recorded waste: ${K.waste}</p><div class="principal-hub-grid"><button id="v96OrderFood">Order 200 meals of stock</button><button id="v96KitchenRepair">Request equipment repair</button><button id="v96KitchenCount">Review today’s meal count</button></div></div>`);$('v96OrderFood').onclick=()=>{K.stock+=200;state.finance.spentOps+=500;v96Record('Cafeteria ordered 200 meals of inventory; $500 charged to operating funds.','Cafeteria');v96Kitchen();};$('v96KitchenRepair').onclick=()=>{K.oven='Repair requested';addTask('Coordinate kitchen equipment repair','Today','High');v96Record('Cafeteria equipment repair requested.','Cafeteria');v96Kitchen();};$('v96KitchenCount').onclick=()=>{let n=Math.round(enrollment()*.75);toast(`${n} projected lunches today; ${K.stock} meals in stock.`);};}
function v96Morale(){v96Init();openModal('👥 Staff Workload & Retention',`<div class="v93modal"><p>Feedback is tied to coverage, changes and follow-through rather than random morale swings.</p>${activeEmployees().map(e=>{let m=V96().morale[e.id];return `<div class="compact-item"><strong>${e.name}</strong> — ${e.position}<br>Workload: ${m.workload>2?'Heavy':m.workload>0?'Elevated':'Normal'} • Assignment stability: ${m.stability<0?'Recent disruption':'Stable'}<br>${m.notes[0]||'No specific concern reported.'} <button data-morale96="${e.id}" class="secondary">Check in</button></div>`;}).join('')}</div>`);document.querySelectorAll('[data-morale96]').forEach(b=>b.onclick=()=>{let e=state.employees.find(x=>x.id===b.dataset.morale96);if(e)v92EmployeeConversation(e);});}
function v96Development(){v96Init();openModal('🎓 Professional Growth & Succession',`<div class="v93modal">${activeEmployees().map(e=>`<div class="compact-item"><strong>${e.name}</strong> • ${e.position}<br>${V96().development.filter(x=>x.employeeId===e.id).map(x=>x.goal+' ('+x.status+')').join(', ')||'No active growth plan'} <button data-dev96="${e.id}" class="secondary">Create growth plan</button></div>`).join('')}</div>`);document.querySelectorAll('[data-dev96]').forEach(b=>b.onclick=()=>{let e=state.employees.find(x=>x.id===b.dataset.dev96);openModal(`Growth plan — ${e.name}`,`<form id="v96DevForm"><label>Goal<select id="v96Goal"><option>Mentor teacher</option><option>Grade-level leader</option><option>Assistant principal pathway</option><option>Head custodian pathway</option><option>Cross-training</option><option>Instructional coaching</option></select></label><button class="primary">Start plan</button></form>`);$('v96DevForm').onsubmit=ev=>{ev.preventDefault();V96().development.unshift({employeeId:e.id,goal:$('v96Goal').value,status:'In progress',days:0});v96Record(`${e.name} began professional growth plan.`,'Staffing');v96Development();};});}
function v96Moves(){v96Init();let moves=V96().classroomMoves;openModal('📦 Physical Classroom Moves',`<div class="v93modal"><p>Teacher assignment changes trigger preparation work. Advance school days to progress moves.</p>${moves.map(x=>`<div class="compact-item"><strong>${x.teacher}</strong> • ${x.from} → ${x.to}<br>${x.stage} • ${x.days} day(s)</div>`).join('')||'<p>No classroom moves pending.</p>'}<button id="v96PlanMoves" class="primary">Generate move tasks from planned assignments</button></div>`);$('v96PlanMoves').onclick=()=>{Object.entries(state.v93?.nextYear||{}).forEach(([id,p])=>{let e=state.employees.find(x=>x.id===id);if(e&&p.room&&e.room!==p.room&&!moves.some(m=>m.employeeId===id&&m.stage!=='Complete'))moves.push({employeeId:id,teacher:e.name,from:e.room,to:p.room,stage:'Packing',days:0});});v96Moves();};}
function v96Families(){v96Init();let V=V96();openModal('👪 Family Communication & History',`<div class="v93modal"><p>Principal promises and contact history are retained across years.</p>${state.families.slice(0,35).map(f=>{let h=V.familyContacts.filter(x=>x.familyId===f.id);return `<div class="compact-item">${f.name||f.lastName+' Family'} • ${h.length} contacts <button class="secondary" data-family96="${f.id}">Contact</button></div>`;}).join('')}</div>`);document.querySelectorAll('[data-family96]').forEach(b=>b.onclick=()=>{let f=state.families.find(x=>x.id===b.dataset.family96);if(!f)return;V.familyContacts.unshift({familyId:f.id,date:state.date,kind:'Principal contact'});v92RememberFamily(f,'Principal proactively contacted family.');v96Record(`Principal contacted ${f.name||f.lastName+' Family'}.`,'Family');v96Families();});}
function v96Annual(){v96Init();let V=V96(),phases={Fall:['Confirm enrollment and room assignments','Review arrival and dismissal','Resolve coverage','Complete opening walkthroughs'],Winter:['Review staff development','Complete evaluation conferences','Draft budget requests','Project enrollment'],Spring:['Collect teacher intentions','Review transfers and vacancies','Interview candidates','Finalize next-year placements'],Summer:['Coordinate classroom moves','Complete deep cleaning','Finalize master schedule','Confirm staffing and registration']};openModal('📅 Annual School Management Planner',`<div class="v93modal"><label>Season<select id="v96Season">${Object.keys(phases).map(x=>`<option ${x===V.phase?'selected':''}>${x}</option>`).join('')}</select></label><div id="v96SeasonTasks"></div></div>`);let draw=()=>{V.phase=$('v96Season').value;$('v96SeasonTasks').innerHTML=phases[V.phase].map(text=>{let x=V.annualTasks.find(t=>t.phase===V.phase&&t.text===text);return `<div class="compact-item"><label><input type="checkbox" data-annual96="${text}" ${x?.done?'checked':''}> ${text}</label></div>`;}).join('');document.querySelectorAll('[data-annual96]').forEach(b=>b.onchange=()=>{let x=V.annualTasks.find(t=>t.phase===V.phase&&t.text===b.dataset.annual96);if(!x){x={phase:V.phase,text:b.dataset.annual96,done:false};V.annualTasks.push(x);}x.done=b.checked;});};$('v96Season').onchange=draw;draw();}
function v96Delegation(){v96Init();let V=V96();openModal('🧭 Department Delegation',`<div class="v93modal"><p>Routine work happens independently unless you require approval. Changing authority affects the principal decision queue.</p>${Object.entries(V.delegation).map(([area,mode])=>`<div class="compact-item"><strong>${area}</strong><select data-delegate96="${area}"><option ${mode==='Routine operations'?'selected':''}>Routine operations</option><option ${mode==='Routine coverage'?'selected':''}>Routine coverage</option><option ${mode==='Routine routing'?'selected':''}>Routine routing</option><option ${mode==='Routine student support'?'selected':''}>Routine student support</option><option ${mode==='Classroom matters'?'selected':''}>Classroom matters</option><option ${mode==='Principal approval'?'selected':''}>Principal approval</option></select></div>`).join('')}</div>`);document.querySelectorAll('[data-delegate96]').forEach(el=>el.onchange=()=>{V.delegation[el.dataset.delegate96]=el.value;v96Record(`Delegation updated for ${el.dataset.delegate96}: ${el.value}`,'Leadership');});}
function v96Day(){v96Init();let V=V96();if(V.lastDaily===state.date)return;V.lastDaily=state.date;v96EnrollmentDay();let lunch=Math.round(enrollment()*.75),K=V.kitchen;K.stock=Math.max(0,K.stock-lunch);K.waste+=Math.round(lunch*.03);V.cafeteriaDays.unshift({date:state.date,lunch,stock:K.stock});if(K.stock<lunch)v96Record(`Kitchen stock low: ${K.stock} meals remain; ${lunch} expected tomorrow.`,'Cafeteria',true,['Approve order']);if(K.oven==='Repair requested'&&Math.random()<.4){K.oven='Working';v96Record('Kitchen equipment repair completed.','Cafeteria');}V.classroomMoves.forEach(x=>{if(x.stage==='Complete')return;x.days++;if(x.days>=2){x.days=0;x.stage=x.stage==='Packing'?'Furniture / custodial':x.stage==='Furniture / custodial'?'Technology / materials':x.stage==='Technology / materials'?'Ready':'Complete';if(x.stage==='Complete')v96Record(`${x.teacher}'s classroom move to ${x.to} is prepared.`,'Staffing');}});V.development.forEach(x=>{if(x.status==='In progress'&&++x.days>=20){x.status='Review due';v96Record(`Professional development review due for ${state.employees.find(e=>e.id===x.employeeId)?.name||'employee'}.`,'Staffing',true,['Schedule review']);}});let absent=state.absences||[];activeEmployees().forEach(e=>{let m=V.morale[e.id];if(!m)return;m.workload=Math.max(0,m.workload-.3);if(absent.some(a=>a.employeeId===e.id))m.notes.unshift('Absence or coverage has affected this team.');});if(Math.random()<.13)v96StaffRequest();for(let i=0;i<rnd(2,4);i++)v96Autonomy();}
function v96Dashboard(){v96Init();let V=V96();openModal('🏫 Autonomous School — V9.6',`<div class="v93modal"><p>Staff handle routine work; you manage priorities, exceptions, staffing and long-term plans.</p><div class="v96kpis"><div><strong>${V.decisions.length}</strong><span>Needs your decision</span></div><div><strong>${V.activity.filter(x=>x.date===state.date&&x.status==='Handled by staff').length}</strong><span>Staff actions today</span></div><div><strong>${V.enrollmentChanges.length}</strong><span>Enrollment changes</span></div></div><div class="principal-hub-grid"><button id="v96Delegate">Delegate departments</button><button id="v96Staff">Staff requests</button><button id="v96Morale">Workload / morale</button><button id="v96Enroll">Enrollment / class sizes</button><button id="v96Bus">Transportation</button><button id="v96Cust">Custodial routes</button><button id="v96Food">Cafeteria</button><button id="v96Grow">Professional development</button><button id="v96Moves">Classroom moves</button><button id="v96Family">Family relationships</button><button id="v96Annual">Annual planner</button><button id="v96DayEnd">Advance operations one day</button></div><h3>Needs principal attention</h3>${V.decisions.slice(0,15).map(x=>`<div class="compact-item">${x.area}: ${x.text}<button class="primary" data-decide96="${x.id}">Review</button></div>`).join('')||'<p>No pending decisions.</p>'}<h3>What happened without you</h3>${V.activity.slice(0,18).map(x=>`<div class="compact-item"><small>${x.date} ${x.time} • ${x.area} • ${x.status}</small><br>${x.text}</div>`).join('')||'<p>Advance the clock or school day to see operations.</p>'}</div>`);
 let links={v96Delegate:v96Delegation,v96Staff:v96StaffRequests,v96Morale:v96Morale,v96Enroll:v96Enrollment,v96Bus:v96Transport,v96Cust:v96Custodial,v96Food:v96Kitchen,v96Grow:v96Development,v96Moves:v96Moves,v96Family:v96Families,v96Annual:v96Annual};Object.entries(links).forEach(([id,fn])=>$(id).onclick=fn);$('v96DayEnd').onclick=()=>{advanceDay();v96Dashboard();};document.querySelectorAll('[data-decide96]').forEach(b=>b.onclick=()=>v96ReviewDecision(b.dataset.decide96));}
function v96StaffRequests(){v96Init();openModal('📨 Employee Requests',`<div class="v93modal">${V96().staffNotices.map(x=>`<div class="compact-item"><strong>${x.name}</strong> • ${x.request}<br>${x.date} • ${x.status} <button class="secondary" data-request96="${x.id}">Meet / respond</button></div>`).join('')||'<p>No requests yet.</p>'}</div>`);document.querySelectorAll('[data-request96]').forEach(b=>b.onclick=()=>{let x=V96().staffNotices.find(y=>y.id===b.dataset.request96),e=state.employees.find(y=>y.id===x.employeeId);x.status='Meeting scheduled';if(e)v92EmployeeConversation(e);else v96StaffRequests();});}
function v96Enrollment(){v96Init();openModal('🏫 Enrollment & Section Planning',`<div class="v93modal"><p>New registrations use available classrooms. Over-capacity rooms require principal attention.</p>${GRADES.map(g=>{let students=state.students.filter(x=>x.status==='Active'&&String(x.grade)===String(g)),rooms=state.rooms.filter(r=>String(r.grade)===String(g));return `<div class="compact-item"><strong>Grade ${g}</strong> • ${students.length} students • ${rooms.length} sections • ${rooms.map(r=>`${r.id}: ${students.filter(s=>s.room===r.id).length}/${r.capacity||24}`).join(' • ')}</div>`;}).join('')}<h3>Recent changes</h3>${V96().enrollmentChanges.slice(0,12).map(x=>`<div class="compact-item">${x.date}: ${x.student} — ${x.kind} ${x.room||''}</div>`).join('')||'<p>No changes yet.</p>'}</div>`);}
function v96Render(){v96Init();let host=$('v95Management');if(!host)return;let box=$('v96Management');if(!box){box=document.createElement('section');box.id='v96Management';box.className='card padded';host.parentNode.insertBefore(box,host.nextSibling);}box.innerHTML=`<div class="section-head"><div><h2>🏫 Autonomous School — V9.6</h2><p class="muted">Delegate operations, review exceptions and plan staffing across years.</p></div><button class="primary" id="v96Open">Open School Management</button></div><div class="v91-strip"><button id="v96QuickDecisions">Decisions: ${V96().decisions.length}</button><button id="v96QuickStaff">Staff Requests</button><button id="v96QuickAnnual">Annual Planner</button></div>`;$('v96Open').onclick=v96Dashboard;$('v96QuickDecisions').onclick=v96Dashboard;$('v96QuickStaff').onclick=v96StaffRequests;$('v96QuickAnnual').onclick=v96Annual;}
const v96RenderOld=render;render=function(){v96Init();v96RenderOld();v96Render();};
const v96TickOld=playableTick;playableTick=function(minutes=5){v96TickOld(minutes);v96Tick();};
const v96AdvanceOld=advanceDay;advanceDay=function(){v96AdvanceOld();v96Day();save(true);};
const v96SaveOld=save;save=function(silent=false){v96Init();try{localStorage.setItem(V96_SAVE_KEY,JSON.stringify(state));}catch(e){console.error(e)}return v96SaveOld(silent);};
const v96LoadOld=load;load=function(){let raw=localStorage.getItem(V96_SAVE_KEY);if(raw){try{state=JSON.parse(raw);v96Init();render();toast('V9.6 autonomous school save loaded.');return;}catch(e){console.error(e)}}return v96LoadOld();};
const v96PrincipalOld=v91PrincipalHub;v91PrincipalHub=function(){v96PrincipalOld();let grid=document.querySelector('.principal-hub-grid');if(grid&&!$('v96PrincipalLink')){let b=document.createElement('button');b.id='v96PrincipalLink';b.textContent='🏫 Autonomous School';b.onclick=v96Dashboard;grid.appendChild(b);}};
...
/* V9.7 — Every Student Matters: persistent student information and support workflows. */
const V97_SAVE_KEY='lincolnElementaryEveryStudentV97';
function v97Init(){v96Init();let V=state.v97=state.v97||{};for(let k of ['interventions','discipline','conferences','supportMeetings','attendanceActions','alumni','improvementGoals'])if(!Array.isArray(V[k]))V[k]=[];V.studentRecords=V.studentRecords||{};V.nurseVisits=V.nurseVisits||[];state.students.forEach(s=>{let r=V.studentRecords[s.id]=V.studentRecords[s.id]||{};r.academic=r.academic||{reading:s.reading||70,math:s.math||70,growth:0};r.behavior=r.behavior||{incidents:0,positive:0};r.supports=r.supports||[];r.contacts=r.contacts||[];r.friends=r.friends||[];r.attendanceHistory=r.attendanceHistory||[];r.notes=r.notes||[];r.enrolled=r.enrolled||state.date;});}
function V97(){v97Init();return state.v97;}
function v97Student(id){return state.students.find(s=>s.id===id);}
function v97Name(s){return s?`${s.first} ${s.last}`:'Unknown student';}
function v97Active(){return state.students.filter(s=>s.status==='Active');}
function v97Search(){v97Init();openModal('🎓 Lincoln Student Information System',`<div class="v93modal"><p>Search persistent student records for academics, attendance, behavior, supports, family contacts and history.</p><input id="v97Q" placeholder="Search student name, grade or room" autofocus><div id="v97Results"></div></div>`);let draw=()=>{let q=$('v97Q').value.trim().toLowerCase(),list=v97Active().filter(s=>!q||`${s.first} ${s.last} ${s.grade} ${s.room}`.toLowerCase().includes(q)).slice(0,60);$('v97Results').innerHTML=list.map(s=>`<button class="v97student" data-stu97="${s.id}"><strong>${v97Name(s)}</strong><span>Grade ${s.grade} • ${s.room||'Unassigned'} • Attendance ${Math.round(s.attendance||100)}%</span></button>`).join('')||'<p>No matching students.</p>';document.querySelectorAll('[data-stu97]').forEach(b=>b.onclick=()=>v97Profile(b.dataset.stu97));};$('v97Q').oninput=draw;draw();}
function v97Profile(id){v97Init();let s=v97Student(id);if(!s)return;let r=V97().studentRecords[id],f=state.families.find(x=>x.id===s.familyId),ints=V97().interventions.filter(x=>x.studentId===id),disc=V97().discipline.filter(x=>x.studentId===id),nurse=V97().nurseVisits.filter(x=>x.studentId===id);openModal(`🎓 ${v97Name(s)} — Student Record`,`<div class="v93modal"><div class="v97tabs"><button id="v97Back" class="secondary">← SIS</button><button id="v97Intervene">Add intervention</button><button id="v97Discipline">Behavior referral</button><button id="v97Conference">Family conference</button><button id="v97Nurse">Nurse visit</button></div><div class="v97grid"><div class="compact-item"><strong>Enrollment</strong><br>Grade ${s.grade} • Room ${s.room||'—'}<br>${s.transport||s.amTransport||'Transportation not recorded'}<br>Family: ${f?.name||'—'}</div><div class="compact-item"><strong>Academic indicators</strong><br>Reading ${r.academic.reading} • Math ${r.academic.math}<br>Growth ${r.academic.growth>=0?'+':''}${r.academic.growth}</div><div class="compact-item"><strong>Attendance</strong><br>${Math.round(s.attendance||100)}%<br>${(s.attendance||100)<90?'⚠ Attendance concern':'On track'}</div><div class="compact-item"><strong>Supports</strong><br>${[s.iep?'IEP':'',s.plan504?'504':'',s.ell?'ELL':'',...r.supports].filter(Boolean).join(' • ')||'No supports recorded'}</div></div><h3>Active interventions</h3>${ints.filter(x=>x.status!=='Closed').map(x=>`<div class="compact-item"><strong>${x.area}</strong> • ${x.status}<br>${x.plan}<br><small>Started ${x.date}; review ${x.review}</small> <button data-review97="${x.id}" class="secondary">Progress monitor</button></div>`).join('')||'<p>None.</p>'}<h3>Behavior / health</h3><p>${disc.length} behavior record(s) • ${nurse.length} nurse visit(s)</p><h3>Family contacts & history</h3>${[...r.contacts.map(x=>`${x.date}: ${x.text}`),...(s.history||[])].slice(0,12).map(x=>`<div class="compact-item">${x}</div>`).join('')||'<p>No history.</p>'}</div>`);$('v97Back').onclick=v97Search;$('v97Intervene').onclick=()=>v97AddIntervention(id);$('v97Discipline').onclick=()=>v97AddDiscipline(id);$('v97Conference').onclick=()=>v97Conference(id);$('v97Nurse').onclick=()=>v97Nurse(id);document.querySelectorAll('[data-review97]').forEach(b=>b.onclick=()=>v97ReviewIntervention(b.dataset.review97));}
function v97AddIntervention(id){let s=v97Student(id);openModal(`📈 Intervention — ${v97Name(s)}`,`<form id="v97IntForm" class="v93form"><label>Area<select id="v97IntArea"><option>Reading</option><option>Math</option><option>Attendance</option><option>Behavior</option><option>Social-emotional</option></select></label><label>Plan<textarea id="v97IntPlan" required>Small-group support and weekly progress monitoring</textarea></label><label>Review in school days<input id="v97IntDays" type="number" value="20" min="5" max="60"></label><button class="primary">Start intervention</button></form>`);$('v97IntForm').onsubmit=e=>{e.preventDefault();let days=+$('v97IntDays').value;V97().interventions.unshift({id:uid('int'),studentId:id,area:$('v97IntArea').value,plan:$('v97IntPlan').value,date:state.date,review:`${days} school days`,days,status:'Active',progress:0});V97().studentRecords[id].notes.unshift(`${state.date}: Intervention started.`);save(true);v97Profile(id);};}
function v97ReviewIntervention(iid){let x=V97().interventions.find(x=>x.id===iid),s=v97Student(x?.studentId);if(!x||!s)return;x.progress+=rnd(1,6);let r=V97().studentRecords[s.id];if(x.area==='Reading')r.academic.reading=clamp(r.academic.reading+rnd(1,3),0,100);if(x.area==='Math')r.academic.math=clamp(r.academic.math+rnd(1,3),0,100);r.academic.growth+=rnd(1,2);if(x.progress>=10)x.status='Monitoring';save(true);v97Profile(s.id);}
function v97AddDiscipline(id){let s=v97Student(id);openModal(`🧭 Behavior Referral — ${v97Name(s)}`,`<form id="v97DiscForm" class="v93form"><label>Concern<select id="v97DiscWhy"><option>Repeated disruption</option><option>Peer conflict</option><option>Unsafe behavior</option><option>Defiance</option><option>Property concern</option></select></label><label>Administrative response<select id="v97DiscAction"><option>Restorative conversation</option><option>Family contact</option><option>Loss of privilege</option><option>Re-entry plan</option><option>Counselor follow-up</option></select></label><button class="primary">Document response</button></form>`);$('v97DiscForm').onsubmit=e=>{e.preventDefault();let why=$('v97DiscWhy').value,action=$('v97DiscAction').value;V97().discipline.unshift({id:uid('dis'),studentId:id,date:state.date,why,action});V97().studentRecords[id].behavior.incidents++;V97().studentRecords[id].contacts.unshift({date:state.date,text:`Behavior follow-up: ${why}; ${action}.`});save(true);v97Profile(id);};}
function v97Conference(id){let s=v97Student(id);V97().conferences.unshift({id:uid('conf'),studentId:id,date:state.date,status:'Scheduled',topic:'Student progress and support'});V97().studentRecords[id].contacts.unshift({date:state.date,text:'Family conference scheduled to review progress and supports.'});addTask(`Family conference: ${v97Name(s)}`,'This week','Normal');save(true);v97Profile(id);}
function v97Nurse(id){let s=v97Student(id),reason=pick(['Headache','Stomach ache','Minor playground injury','Medication visit','Not feeling well']);V97().nurseVisits.unshift({id:uid('nur'),studentId:id,date:state.date,time:mins12(state.simMinutes),reason,outcome:'Returned to class'});V97().studentRecords[id].notes.unshift(`${state.date}: Nurse visit — ${reason}.`);save(true);v97Profile(id);}
function v97Attendance(){v97Init();let list=v97Active().filter(s=>(s.attendance||100)<92).sort((a,b)=>a.attendance-b.attendance);openModal('📅 Attendance & Early Warning',`<div class="v93modal"><p>Students below 92% are shown for review. Below 90% is treated as a stronger concern in this simulation.</p>${list.map(s=>`<div class="compact-item"><strong>${v97Name(s)}</strong> • Grade ${s.grade} • ${Math.round(s.attendance)}% <button data-att97="${s.id}" class="secondary">Create follow-up</button></div>`).join('')||'<p>No students currently below the review threshold.</p>'}</div>`);document.querySelectorAll('[data-att97]').forEach(b=>b.onclick=()=>{let s=v97Student(b.dataset.att97);V97().attendanceActions.unshift({id:uid('att'),studentId:s.id,date:state.date,action:'Family outreach and attendance check-in',status:'Open'});V97().studentRecords[s.id].contacts.unshift({date:state.date,text:'Attendance outreach initiated.'});addTask(`Attendance follow-up: ${v97Name(s)}`,'This week','High');save(true);v97Attendance();});}
function v97Supports(){v97Init();let list=v97Active().filter(s=>s.iep||s.plan504||s.ell||V97().interventions.some(x=>x.studentId===s.id&&x.status!=='Closed'));openModal('🤝 Student Support Services',`<div class="v93modal"><p>Caseload view for IEP, 504, ELL and intervention supports.</p>${list.slice(0,80).map(s=>`<button class="v97student" data-support97="${s.id}"><strong>${v97Name(s)}</strong><span>${[s.iep?'IEP':'',s.plan504?'504':'',s.ell?'ELL':'',V97().interventions.some(x=>x.studentId===s.id&&x.status!=='Closed')?'Intervention':''].filter(Boolean).join(' • ')}</span></button>`).join('')||'<p>No current support records.</p>'}</div>`);document.querySelectorAll('[data-support97]').forEach(b=>b.onclick=()=>v97Profile(b.dataset.support97));}
function v97Improvement(){v97Init();let V=V97(),att=Math.round(v97Active().reduce((a,s)=>a+(s.attendance||100),0)/Math.max(1,v97Active().length)),read=Math.round(v97Active().reduce((a,s)=>a+(V.studentRecords[s.id]?.academic.reading||s.reading||0),0)/Math.max(1,v97Active().length));openModal('📊 School Improvement — Student Outcomes',`<div class="v93modal"><div class="v96kpis"><div><strong>${att}%</strong><span>Attendance</span></div><div><strong>${read}</strong><span>Reading indicator</span></div><div><strong>${V.discipline.length}</strong><span>Behavior records</span></div></div><form id="v97GoalForm" class="v93form"><label>New improvement goal<input id="v97Goal" required placeholder="Example: Improve Grade 3 reading growth"></label><button class="primary">Add goal</button></form><h3>Goals</h3>${V.improvementGoals.map(g=>`<div class="compact-item"><strong>${g.text}</strong><br>${g.date} • ${g.progress}% progress <button data-goal97="${g.id}" class="secondary">Review progress</button></div>`).join('')||'<p>No goals yet.</p>'}</div>`);$('v97GoalForm').onsubmit=e=>{e.preventDefault();V.improvementGoals.unshift({id:uid('goal'),date:state.date,text:$('v97Goal').value,progress:0});save(true);v97Improvement();};document.querySelectorAll('[data-goal97]').forEach(b=>b.onclick=()=>{let g=V.improvementGoals.find(x=>x.id===b.dataset.goal97);g.progress=clamp(g.progress+rnd(2,8),0,100);save(true);v97Improvement();});}
function v97Dashboard(){v97Init();let V=V97();openModal('🎓 Every Student Matters — V9.7',`<div class="v93modal"><p>Student records now connect academics, attendance, behavior, supports, family communication, health visits and long-term history.</p><div class="v96kpis"><div><strong>${v97Active().length}</strong><span>Active students</span></div><div><strong>${V.interventions.filter(x=>x.status!=='Closed').length}</strong><span>Active interventions</span></div><div><strong>${v97Active().filter(s=>(s.attendance||100)<90).length}</strong><span>Attendance concerns</span></div></div><div class="principal-hub-grid"><button id="v97SIS">Student Information System</button><button id="v97Att">Attendance / early warning</button><button id="v97Sup">Support services / MTSS</button><button id="v97Imp">School improvement</button></div><h3>Recent student support activity</h3>${[...V.interventions.slice(0,5).map(x=>`${x.date}: ${v97Name(v97Student(x.studentId))} — ${x.area} intervention`),...V.conferences.slice(0,5).map(x=>`${x.date}: ${v97Name(v97Student(x.studentId))} — family conference`)].slice(0,10).map(x=>`<div class="compact-item">${x}</div>`).join('')||'<p>No new activity yet.</p>'}</div>`);$('v97SIS').onclick=v97Search;$('v97Att').onclick=v97Attendance;$('v97Sup').onclick=v97Supports;$('v97Imp').onclick=v97Improvement;}
function v97Day(){v97Init();let V=V97();v97Active().forEach(s=>{let r=V.studentRecords[s.id];if(Math.random()<.018){s.attendance=clamp((s.attendance||100)-rnd(0,1),70,100);r.attendanceHistory.unshift({date:state.date,status:'Absent'});}if(Math.random()<.025){r.academic.reading=clamp(r.academic.reading+rnd(-1,2),0,100);r.academic.math=clamp(r.academic.math+rnd(-1,2),0,100);}if(Math.random()<.006)r.behavior.positive++;});V.interventions.filter(x=>x.status==='Active'||x.status==='Monitoring').forEach(x=>{x.days--;if(x.days<=0){x.status='Review due';let s=v97Student(x.studentId);state.inbox.unshift(msg('Student Support Team',`Intervention review due: ${v97Name(s)}`,`${x.area} intervention is ready for progress review.`,'medium',['Acknowledge']));}});}
function v97Render(){v97Init();let host=$('v96Management');if(!host)return;let box=$('v97Management');if(!box){box=document.createElement('section');box.id='v97Management';box.className='card padded';host.parentNode.insertBefore(box,host.nextSibling);}box.innerHTML=`<div class="section-head"><div><h2>🎓 Every Student Matters — V9.7</h2><p class="muted">Persistent student records, interventions, attendance, behavior, health, family contacts and school improvement.</p></div><button class="primary" id="v97Open">Open Student Management</button></div><div class="v91-strip"><button id="v97QuickSIS">Student SIS</button><button id="v97QuickAtt">Attendance</button><button id="v97QuickSup">Supports</button></div>`;$('v97Open').onclick=v97Dashboard;$('v97QuickSIS').onclick=v97Search;$('v97QuickAtt').onclick=v97Attendance;$('v97QuickSup').onclick=v97Supports;}
const v97RenderOld=render;render=function(){v97Init();v97RenderOld();v97Render();};
const v97AdvanceOld=advanceDay;advanceDay=function(){v97AdvanceOld();v97Day();save(true);};
const v97SaveOld=save;save=function(silent=false){v97Init();try{localStorage.setItem(V97_SAVE_KEY,JSON.stringify(state));}catch(e){console.error(e)}return v97SaveOld(silent);};
const v97LoadOld=load;load=function(){let raw=localStorage.getItem(V97_SAVE_KEY);if(raw){try{state=JSON.parse(raw);v97Init();render();toast('V9.7 Every Student Matters save loaded.');return;}catch(e){console.error(e)}}return v97LoadOld();};
const v97PrincipalOld=v91PrincipalHub;v91PrincipalHub=function(){v97PrincipalOld();let grid=document.querySelector('.principal-hub-grid');if(grid&&!$('v97PrincipalLink')){let b=document.createElement('button');b.id='v97PrincipalLink';b.textContent='🎓 Student Information & Support';b.onclick=v97Dashboard;grid.appendChild(b);}};
